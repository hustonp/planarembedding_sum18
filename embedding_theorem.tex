...
Ordinarily, for subfactors and planar algebras, Kauffman diagrams are drawn with strings going from \emph{bottom to top}. We put the number $k$ next to a strand to denote a bundle of $k$ parallel strands, and the label is omitted for single strands.

\nn{where we do the associativity calculations:
\\
In this section, we rotate the diagrams by $\pi/2$ in the \emph{counterclockwise} direction, so that diagrams are read going from \emph{right to left}.
This has the advantage that we read the diagram as an element in a von Neumann algebra with multiplication written as usual, and it helps us save space.
For example, the generator $E_i$ in $\cT\cL\cJ_{n,\pm}$ for $1\leq i\leq n-1$ is depicted by:
$$
E_i
=
...
E_i
:=
\begin{tikzpicture}[baseline = .6cm, rotate=90]
	\draw[thick, rounded corners = 5pt] (0,0) rectangle (1.4,1);
	\draw (.2,0) -- (.2,1);
	\draw (1,0) -- (1,1);
	\draw (.4,1) arc (-180:0:.2cm);
	\draw (.4,0) arc (180:0:.2cm);
%	\node at (.4,1.2) {\scriptsize{$1$}};
%	\node at (.4,-.2) {\scriptsize{$1$}};
	\node at (.35,.5) {\scriptsize{$i$}};
	\node at (1.2,.5) {\scriptsize{$n{-}i{-}2$}};
\end{tikzpicture}
$$
}
\end{nota}
...
In Proposition \ref{prop:MarkovCstar} below, we verify $\cM_0$ is a $\Cstar$ category under this dagger structure.\footnote{
...
\begin{pmatrix}
\begin{tikzpicture}[baseline = .3cm]
	\draw (.2,.6) arc (-180:0:.3cm);
	\draw (.2,-.6) arc (180:0:.3cm);
	\draw (.4,.6) arc (-180:0:.1cm);
	\draw (.4,-.6) arc (180:0:.1cm);
	\draw (0,-.6) -- (0,.6);
...
\begin{tikzpicture}[baseline = .3cm]
	\draw (.2,.6) arc (-180:0:.3cm);
	\draw (.4,.6) arc (-180:0:.1cm);
	\draw (.2,-.6) -- (.2,0) arc (180:0:.2cm) -- (.6,-.6);
	\draw (.8,-.6) arc (180:0:.2cm);
	\draw (0,-.6) -- (0,.6);
	\roundNbox{unshaded}{(0,-.2)}{.2}{0}{.2}{}
	\node at (0,-.8) {\scriptsize{$n$}};
	\node at (0,.8) {\scriptsize{$n$}};
	\node at (.2,.8) {\scriptsize{$j$}};
	\node at (.4,.8) {\scriptsize{$k$}};
...
\begin{tikzpicture}[baseline = .3cm]
	\draw (.2,.6) arc (-180:0:.3cm);
	\draw (.4,.6) arc (-180:0:.1cm);
	\draw (.2,-.6) -- (.2,-.1) arc (180:0:.3cm) -- (.8,-.6);
	\draw (.4,-.6) -- (.4,-.1) arc (180:0:.1cm) -- (.6,-.6);
	\draw (0,-.6) -- (0,.6);
	\roundNbox{unshaded}{(0,-.3)}{.2}{0}{.3}{}
	\node at (0,-.8) {\scriptsize{$n$}};
	\node at (0,.8) {\scriptsize{$n$}};
	\node at (.2,.8) {\scriptsize{$j$}};
	\node at (.4,.8) {\scriptsize{$k$}};
...
\end{tikzpicture}
\end{pmatrix}
\nn{\text{todo: thinking about submultiplicativity of the norm}}
$$
\end{itemize}
%
%
