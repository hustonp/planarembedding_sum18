...
\begin{enumerate}[label={\rm(SF\arabic*)}]
 \item Each disc and semidisc, including the output semidisc, is rectangular in shape, with an equal number of strings emerging from the top and bottom,
...
Given a shaded planar module tangle $T$ of type \nn{define type}
...
Each time the horizontal line passes a disc, semidisc, or local extrema (which can happen only one at a time!), we compose with a morphism from $\cM$.
In more detail, when the horizontal line passes:
\begin{itemize}
\item
the $i$-th input semidisc with vertical stands to the right, we compose with $f_j \vartriangleleft \id$, where $\id$ is the appropriate identity morphism corresponding to the strands to the right of the input semidisc
\item
the $j$-th input disk with vertical strands to the left and right, we compose with $\id_{m}\vartriangleleft \id_l \otimes x \otimes \id_r$ where $\id_l, \id_r$ correspond to the  appropriate identity morphisms corresponding to the strands to the left and right of the input disc
\item
...
For $f:[n]\to[m]$ where $m<n$, we define $f\vartriangleleft 1$ by taking dagger. 
 ...
 Let $\iota$ denote the inclusion in $M_\bullet$. 
 ...
 \begin{align*}
  ...
  \right)
  \\&= 
  E^{n+j+4k+i}_{n+j+3k+i}\left(\iota(f)\circ E^{n+j+4k+2i}_{n+j+4k+i}\left(
  \begin{tikzpicture}[baseline=.3cm]
   \begin{scope}[shift={(0,1)}]
    \begin{scope}[shift={(.1,0)}]
     \draw (.1,0) -- (.1,.8);
 ...
 \end{tikzpicture}
 \circ\iota(f)\circ E^{n+j+4k+2i}_{n+j+4k+i}\left(
  % \TLPlusPIso{n}{k}{j+2k+i}
...
\begin{rem}
 ...
 In order to give the category of projections of a Markov tower the structure of a pivotal right $\TLJ$-module, it will be helpful to present $\TLJ$ as a category of projections. Reassuringly, the category of projections $\cT$ of the Markov tower ${TLJ}_\bullet$ is $\dag$-tensor equivalent to the tensor category $\TLJ$, with the internal tensor product given by the action of $\TLJ$ on itself. 
...
\nn{the factors of $d$ are off here!}
$$
