...
$$
where $(\overline{c}, \ev_c, \coev_c)$ is the canonical \emph{balanced} dual of $c\in \cC$ \cite{MR1444286,MR2091457,MR3342166,UnitaryDualFunctors}.
...
Now starting with a Markov tower, we can take its category of projections, defined as follows.


\begin{defn}
Let $M_\bullet = (M_n, \tr_n, e_{n+1})$ be a Markov tower.
Let $\cM_0$ be the category defined as follows.
\begin{itemize}
\item
...
Notice that if $j=k=0$, then this definition of $x^\dag \circ y^\dag$ agrees with the above definition of the composite, since $E^{n+2j+k}_{n+j+k}$ preserves the adjoint and the Temperley-Lieb-Jones element above is the identity.
...
Notice that if $\ell=0$, then this definition of $x^\dag \circ y^\dag$ agrees with the above definition of the composite, since $E^{n+2\ell + k}_{n+\ell}$ preserves the adjoint and the Temperley-Lieb-Jones element above is the identity.
...
Notice that if $\ell=0$, then this definition of $x^\dag \circ y^\dag$ agrees with the above definition of the composite, since the Temperley-Lieb-Jones element above is the identity.
\end{enumerate}
\end{itemize}
\nn{show composition is associative.}

Finally, we let $\cM$ be the Karoubi completion of $\cM$.
\end{defn}


Starting with a cyclic right planar module $M_\bullet$ for the subfactor planar algebra $\cQ_\bullet$, we take its category of projections.



\nn{converse construction of module from a right planar module.}
