...
\maketitle
\begin{abstract}
 Jones and Penneys have shown that a finite depth subfactor planar $\dag$-algebra embeds in the bipartite graph planar algebra of its principal graph, via a Markov towers of algebras approach.
 We relate several equivalent perspectives on the notion of modules over a subfactor planar $\dag$-algebra, and show that a Markov tower is equivalent to a module over the Temperley-Lieb planar algebra.
 As a corollary, we obtain a classification of semisimple pivotal $\Cstar$-modules over Temperley-Lieb in terms of pointed graphs with a Perron vertex weighting.
 We then generalize the Markov towers of algebras approach to describe how a finite depth subfactor planar $\dag$-algebra embeds in the bipartite graph planar algebra of the fusion graph of any of its cyclic modules.
\end{abstract}
...
We begin with the following lemma.
 ...
 \begin{lem}
 \label{lem:LeftKink}
  \nn{From here, the width of the usual partial isometry between Jones projections is inconsistntly sized. Bikeshed.}
...
For $f:[n]\to[m]$ where $m<n$, we define $f\vartriangleleft 1$ by taking dagger. 
 ...
 \begin{align*}
  (f\vartriangleleft 1)\circ(1\vartriangleleft g) &= E^{n+j+4k+2i}_{n+j+3k+i}\left(\iota(f)\cdot d^{-(i+k)}
  ...
  \right)
  \\&= 
  E^{n+j+4k+i}_{n+j+3k+i}\left(\iota(f)\cdot E^{n+j+4k+2i}_{n+j+4k+i}\left(d^{-(i+k)}
  \begin{tikzpicture}[baseline=.3cm]
   \begin{scope}[shift={(0,1)}]
    \begin{scope}[shift={(.1,0)}]
     \draw (.1,0) -- (.1,.8);
 ...
 \end{tikzpicture}
 \cdot\iota(f)\cdot E^{n+j+4k+2i}_{n+j+4k+i}\left(d^{-(i+k)}
  % \TLPlusPIso{n}{k}{j+2k+i}
...
% \TLPlusPIso{n+j+k}{k}{i+k}
  ...
  \end{tikzpicture}
\right)\\
&=d^{-2i}(1\vartriangleleft g)\circ(f\vartriangleleft 1)
 \end{align*}
 \endgroup
 ...
 We describe a representative case. Suppose $f:[n]_{\cM}\to[n+2i+2j]_{\cM}$ and $g:[n+2i+2j]_{\cM}\to[n+2i]_{\cM}$, and suppose $m=k+i+j$. Then
 \begin{align*}
  (g\vartriangleleft 1_m)\circ (f\vartriangleleft 1_m) &= E^{n+3i+k+2j}_{n+2i+k+j}\left(
  \begin{tikzpicture}[baseline=.3cm]
   \draw (.2,0) -- (.2,.8);
   \node at (.2,1) {\scriptsize{$n$}};
   \draw (.4,0) -- (.4,.8);
   ...
   \draw[thick, rounded corners = 5pt] (0,0) rectangle (1.8,.8);
  \end{tikzpicture}
  \cdot\iota(g)\cdot\iota(f)\cdot d^{-i}
  % \rho\left(
  \begin{tikzpicture}[baseline=.3cm]
  ...
  \end{tikzpicture}
  \right)
  \\&= E^{n+3i+k+2j}_{n+2i+k+j}\left(d^{-j}
  \begin{tikzpicture}[baseline=.3cm]
   \begin{scope}[shift={(0,.4)}]
    \draw (.2,0) -- (.2,.8);
    \node at (.2,1) {\scriptsize{$n$}};
  ...
  \end{tikzpicture}
  \cdot\iota(g)\cdot\iota(f)\cdot d^{-(2i+j)}
  \begin{tikzpicture}[baseline=.3cm]
  ...
  \end{tikzpicture}
  \right)
  \\&= E^{n+3i+k+2j}_{n+2i+k+j}\left( % d^{-j} dissapears. Calculate it; E_n and e_n have a factor of d^{-1}, the diagram we draw is de_n rather than e_n, but we still use the regular E_n. So changing de for E means a factor of d appears.
  \begin{tikzpicture}[baseline=.3cm]
   \draw (.2,0) -- (.2,.8);
   \node at (.2,1) {\scriptsize{$n$}};
   \draw (.4,0) -- (.4,.8);
  ...
  \begin{tikzpicture}[baseline=.3cm]
   ...
   \draw[thick, rounded corners=5pt] (0,0) rectangle (1.4,.8);
  \end{tikzpicture}
  \right)\right)\cdot d^{-(2i+j)}E^{n+3i+k+2j}_{n+2i+k+j}\left(
  \begin{tikzpicture}[baseline=.3cm]
   \begin{scope}[shift={(0,.8)}]
    \draw (.2,0) -- (.2,.8);
    \node at (.2,1) {\scriptsize{$n$}};
  ...
  \begin{tikzpicture}[baseline=.3cm]
   ...
   \draw[thick, rounded corners=5pt] (0,0) rectangle (1.4,.8);
  \end{tikzpicture}
  \right)\right)\cdot d^{-3i} % applying E and removing bubbles gives -i-j+2j
  \begin{tikzpicture}[baseline=.3cm]
   \draw (.2,0) -- (.2,.8);
   \node at (.2,1) {\scriptsize{$n$}};
   \draw (.4,.8) arc (-180:0:.2cm);
  ...
  \begin{tikzpicture}[baseline=.3cm]
   ...
   \draw[thick, rounded corners=5pt] (0,0) rectangle (1,.8);
  \end{tikzpicture}
  \right)\right)\cdot d^{-i} % Lemma creates d^i
  \begin{tikzpicture}[baseline=.3cm]
   \begin{scope}[shift={(0,.4)}]
    \draw (.2,0) -- (.2,.8);
    \node at (.2,1) {\scriptsize{$n$}};
   ...
   \draw[thick, rounded corners=5pt] (0,-.4) rectangle (1.6,1.2);
  \end{tikzpicture}
  \\&= \iota\left(E^{n+2i+j}_{n+i}\left(g\cdot\iota(f)\cdot % now all d are gone, because the last step removed an $i$-fold bubble
  \begin{tikzpicture}[baseline=.3cm]
   \draw (.2,0) -- (.2,.8);
   \node at (.2,1) {\scriptsize{$n$}};
   \draw (.6,.8) .. controls ++(-90:.3cm) and ++ (-270:.3cm) .. (1,0);
  ...
  \begin{tikzpicture}[baseline=.3cm]
   ...
   \draw[thick, rounded corners=5pt] (0,0) rectangle (1.6,.8);
  \end{tikzpicture}
  \\&= d^i(g\circ f)\vartriangleleft1_m
 \end{align*}

 It is easier to check that the action is associative.
 Again, since we have already shown that $\vartriangleleft$ is functorial, it suffices to check associativity of triples of morphisms when two are the identity. That $1\vartriangleleft(1\otimes h)=(1\vartriangleleft1)\vartriangleleft h$ and that $1\vartriangleleft(g\otimes 1)=(1\vartriangleleft g)\vartriangleleft 1$ for all $g,h\in\TLJ$ follow directly from the definitions of $\vartriangleleft$ and $\otimes$. Finally, in case $f\in\cM([n]\to[n+2i])$, we have 
...
\begin{enumerate}[label={\rm(\arabic*)}]
\item
The projection $p\vartriangleleft \id_{X^{\text{alt}\otimes 2k}}$ has central support 1 in the finite dimensional von Neumann algebra $M_{2(j+k)} = \End_\cM(m \vartriangleleft X^{\text{alt}\otimes 2k})$, which is equivalent to $M_{2(k+j)} = M_{2(k+j)}p M_{2(k+j)}$ by finite dimensionality.
\item
