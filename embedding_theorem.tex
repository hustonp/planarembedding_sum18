...
\newcommand{\TLTCapcup}[1]{
	\TLTCalcLabelOffset[.6cm]{\TLTlabelwidth}{#1}
		\coordinate (TLTlead) at ($ (TLTlead) + ({\TLTlabelwidth},0) $);
	\begin{scope}[shift=(TLTlead)]
...
\newcommand{\TLTSnakeR}[2]{
	...
	\newlength{\TLTsswidth}
	\TLTCalcLabelOffset[.7cm]{\TLTscwidth}{#1}
	\TLTCalcLabelOffset[.3cm]{\TLTsswidth}{#2}
	\setlength{\TLTlabelwidth}{\maxof{\TLTscwidth}{\TLTsswidth}}
	\setlength{\TLTlabelwidth}{\maxof{\TLTlabelwidth}{.7cm}} % because of optional value to first offset above, noop
	\begin{scope}[shift=(TLTlead)]
	...
	\end{scope}
	\coordinate (TLTlead) at ($ (TLTlead) + ({2\TLTlabelwidth},0) $);
}
...
We build a connected right planar $\cP_\bullet$-module $\cM_\bullet$ by defining $\cM_k := \End_\cC( m \vartriangleleft X^{\text{alt}\otimes k})$ for $k\geq 0$, and we define the action of the shaded planar module operad via the diagrammatic calculus for $\cM$.
\nn{describe how to do this.}
\end{defn}
...
\nn{Is there an older source for this term?} 
