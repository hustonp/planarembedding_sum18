...
In our final Section \ref{sec:InvarianceOfEmbedding}, we explain that picking a different simple $m'\in \cM$ or a different $r'\geq 0$, we get an equivalent planar $\dag$-algebra embedding $\cP_\bullet \to \cG\cP\cA(\Gamma)_\bullet$.
...
We will see that, under the correspondance of Theorem \ref{thm:ModuleEquivalence}, connected right planar modules correspond to cyclic pivotal right module $\Cstar$-categories.
\end{defn}
...
\end{align*}
 The $\TLJ$ action on $\cM$ comes from the fact that $TLJ$ is the initial Markov tower for a given parameter. % say more?
...
Suppose $M_\bullet$ is a a connected right planar module over $Q_\bullet$. 
Since $Q_\bullet$ has finite depth, so does $M_\bullet$, by Lemma \ref{lem:FiniteDepthAlgebraHasFiniteDepthModule}. The tangle 
 \[\begin{tikzpicture}[baseline]
  \halfcircle{}{(0,0)}{.5}{.5}{}
  \draw (.5,-.5) -- (.5,-.25);
  \draw (.5,.25) -- (.5,.5);
  ...
  \ncircle{}{(.5,0)}{.25}{180}{}
 \end{tikzpicture}\]
gives a map $Q_\bullet\to M_\bullet$, which is injective since $M$ is non-zero and cyclic. % Is this enough?
If we choose $r$ so that the inclusion $M_{2r}\subseteq M_{2r+1}$ is strongly Markov and set $B_n=M_{2r+n}$, then as described in section \ref{ssec:shift}, the tower $(B_n,\tr_{2r+n})$ is a strongly Markov tower. By Theorem 3.28 of \cite{MR2812459}, % it may be possible to shorten this reference, to the summary earlier in the paper. 
the relative commutants of this tower are isomorphic to the box spaces of the canonical shaded $*$-planar algebra $P_\bullet$ of the Bratteli diagram of $B_0\subseteq B_1$. Explicitly, 
\begin{align*}
 P_{n,+} &=  B'_{0}\cap B_{n}\text{ and } \\
 P_{n,-}  &= B'_{1}\cap B_{n+1}\text{.}
\end{align*}
Therefore, the tangle 
 \[\begin{tikzpicture}[baseline]
  ...
  \ncircle{}{(.9,0)}{.25}{180}{}
 \end{tikzpicture}\]
gives an embedding $\Phi:Q_n\to P_n$ on the level of Markov towers.
In terms of the string calculus of pivotal modules over tensor categories, $\Phi$ places $2r$ strings to the left of elements in $Q_{n,+}$  and $2r+1$ strings to the left of elements in $Q_{n,-}$:
\[ \begin{tikzpicture}[baseline]
 \draw (0,-.7) -- (0,.7);
 \roundNbox{unshaded}{(0,0)}{.3}{0}{0}{$x$}
 \node at (0,-.9) {\tiny{$n$}};
...
\begin{tikzpicture}[baseline]
 ...
 \node at (0,-0.9){\tiny{$2r$}};
\end{tikzpicture} \]
Here, the unshaded-shaded strand is a simple generator of the category of projection $\cC$ of $Q_\bullet$, while the shaded-unshaded red strand is a generator of a cyclic pivotal right module category over $\cC$. 

It remains to use Lemma \ref{lem:SufficientConditionsForPlanarMap} to show that this embedding is a morphism of $\dag$-planar algebras.
\begin{thm}
...
This means that, for any $x \in Q_{n,+}$, we have 
...
In particular, by considering $(Q_\bullet,1)$ as a cyclic right module over itself, we recover the main result of \cite{MR2812459}, and by instead considering $Q_\bullet$ as a left module, we obtain an embedding into the graph planar algebra of the dual principle graph. 
