...
By a \emph{pivotal module category} for $\cQ_\bullet$, we mean a finitely semisimple $\Cstar$ category $\cM$ which is an indecomposable right $\cC$-module category, together with a faithful positive trace $\Tr^\cM_m$ on each endomorphism $\Cstar$ algebra $\End_\cM(m)$ which is compatible with the left $\cC$-action (see \nn{}).
...
$$
where $(\overline{c}, \ev_c, \coev_c)$ is the canonical \emph{balanced} dual of $c\in \cC$ \nn{}.
...
\begin{thmalpha}
The unital $*$-algebra maps $\Phi_{n,\pm} : \cQ_{n,\pm} \to \cP_{n,\pm}$ given by adding $\id_m \vartriangleleft \id_{(c\otimes \overline{c})^{\otimes r}}$ to the left
$$
\begin{tikzpicture}[baseline]
	\draw (0,-.7) -- (0,.7);
