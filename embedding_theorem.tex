...
Given a Markov sequence $(M_n,\tr_n, e_n)$, no additional assumptions on $p\in P(M_0)\setminus\{0\}$ are necessary to obtain another Markov sequence by compression.
...
\begin{cor}
\label{cor:OntoCor}
If $x$ belongs to $A'_k \cap A_{n+k}$ then we have that $x$ can represented in the following form as a string diagram in the box space $A'_0\cap A_{n+k}$ for the canonical planar $\ast$-algebra for $A_0\subseteq (A_1,\tr_1)$:
\[
...
\end{cor}
\begin{proof} 
For $x\in A'_k\cap A_{n+k}$ we have:
\[
\begin{tikzpicture}[baseline]
\draw (0,-.7) -- (0,.7);
