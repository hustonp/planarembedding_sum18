...
together with an action of the shaded planar module operad
such that
\nn{todo}
\end{defn}
...
\begin{ex}
Given a subfactor planar algebra $\cP_\bullet$, the even parts $\cP_\pm :=( \cP_{k,\pm})_{k\geq 0}$ are each separately cyclic right planar modules for $\cP_\bullet$.
\end{ex}
...
\right)
:=
Z_{\cG_\bullet}(\widetilde{\cT})(m_1\otimes \cdots \otimes m_r \otimes \Phi(x_1)\otimes \cdots\otimes \Phi(x_s))
$$
...
and inserting one copy of $p_v$ in the left-most region of the new tangle.
We illustrate this procedure on the tangle from \nn{above}:
$$
\cT=
\begin{tikzpicture}[baseline = -.1cm]
	\filldraw[shaded] (.3,1.3) .. controls ++(90:.3cm) and ++(270:.3cm) .. (.8,2) -- 
...
%	\halfcircle{}{(0,0)}{2}{2}{}
%
	\ncircle{unshaded}{(-.2,0)}{.25}{180}{\scriptsize{$p_v$}}
	\ncircle{unshaded}{(.3,1)}{.4}{180}{\scriptsize{$2^{\cM_\bullet}$}}
	\ncircle{unshaded}{(.4,-1)}{.4}{180}{\scriptsize{$1^{\cM_\bullet}$}}
	\ncircle{unshaded}{(2.5,1)}{.4}{180}{\scriptsize{$3^{\cP_\bullet}$}}
	\ncircle{unshaded}{(1.5,0)}{.4}{0}{\scriptsize{$2^{\cP_\bullet}$}}
	\ncircle{unshaded}{(2.5,-1)}{.4}{90}{\scriptsize{$1^{\cP_\bullet}$}}
