...
\newcommand{\RelativeCommutantShading}{blue!15!white}
\newcommand{\CTwoShading}{red!15!white}
...
\nn{add unitarity to below.}
By a module over $Q_\bullet$, we mean a module category over the multitensor category $\cC$. Let $\cM$ be a cyclic left $\cC$-module category with generator $m$. By \cite{ostrik03}, % think more about who to cite maybe?
there is an algebra $A$ internal to $\cC$ such that $\cM\cong\FreeMod_\cC(A)$, the category of free right $A$-modules internal to $\cC$. 
\nn{this algebra must be a Q-system, i.e., you need to make it unitary. 
Cite \cite[Thm.~A.1]{1704.04729}}
...
We can again obtain a $3\times 3$ multifusion category $\widetilde{\cC}$ from the $2$-category of bimodules over the algebras $1_{0,+}$, $1_{0,-}$, and $A$, into which both $\cC$ and $\cM$ have naturally monoidal embeddings. 
...
It follows that a Markov sequence has finite depth if and only if there is $n\in \bbN$ such that $Y_n = (0)$, as in \ref{prop:ElementaryMarkov} \eqref{EP:FiniteDepth}. Let $(M_n)$ be a Markov sequence with finite depth, and let $n\in \bbN$ be such that $Y_n=(0)$. Now notice that for $k<n$ the Bratelli diagram of $M_k\subseteq M_{k+1}$ is the Bratelli diagram of $M_{k-1}\subseteq M_{k}$ reflected upwards along with additional edges which are part of the principal graph. Because of this and the fact that the Bratteli diagram for $M_0\subseteq M_1$ is part of the principal graph we can ``unravel" the Bratelli diagram for $M_{n}\subseteq M_{n+1}$ to obtain the principal graph for the Markov sequence $(A_n,tr_n)$.
...
Given a subfactor planar algebra $Q_{\bullet}$, let $\widetilde{\cC}$, and $\cC$ be the multifusion categories  associated to $\cG$ and $\cM$ (the paragroup for $Q_{\bullet}$ and a module category for $Q_{\bullet}$) as described in section 2.1. Let $1_{\widetilde{\cC}}= 1_0 \oplus 1_1 \oplus 1_2$ be the tensor unit in $\widetilde{\cC}$ and $\cC$ the  non-unital $2\times 2$ unitary multifusion subcategory with $1_\cC = 1_0\oplus 1_1$, so that $\cM\cong\cC_{20}\oplus \cC_{21}$. Overall,
$$
\cM 
=
\begin{pmatrix}
...
\begin{tikzpicture}[baseline=-.1cm]
	...
	\fill[\RelativeCommutantShading] (0,-.7) rectangle (.7,.7);
	\draw (0,-.7) -- (0,.7);
...
\begin{tikzpicture}[baseline=-.1cm, rotate=90]
	\filldraw[thick, red, fill=\CTwoShading] (-.3,.15) arc (270:90:.2cm) -- (.3,.55) arc (90:-90:.2cm);
...
\begin{proof}
	...
	\[m\otimes \xalt \cong \bigoplus\limits_{j}n_jY_j \]
 For some nonnegative integers $n_1,\ldots n_r$ so then we have:
	\[\End_{\cC}(m\otimes\xalt)\cong \bigoplus\limits_{j}\End_{\cC}(n_j Y^l_j) \]
	If there are $s$ ismorphism classes of simple objects in $\cC_{21}$ we obtain a similar result. So for any $n$ the Bratellia diagram for $A_n\subseteq A_{n+1}$ has at most $r+s$ vertices, so the principal graph must be finite. 
\end{proof}
...
First, recall that for a finite depth Markov sequence, the principal graph is canonically isomorphic to the Bratteli for the inclusion $A_n\subseteq A_{n+1}$ for $n$ large enough by \ref{BratteliPrincipal}. So, it suffices to show this Bratteli diagram is isomorphic to the fusion graph for $\cM$. Furthermore if $n$ is such that $A_n\subseteq A_{n+1}\subseteq A_{n+2}$ is standard then for $k\geq n$ the Bratelli diagram for $A_{k}\subseteq A_{k+1}$ is isomorphic to the Bratelli diagram for $A_n\subseteq A_{n+1}$ so without loss of generality we may assume that $n$ is even.
...
where each $\phi_j \in \End_{\cC}(n_jY_j)$. Then, distributing tensor products over the sum, we may write $\phi$ as 
$$\bigoplus_{j}(\phi_j \otimes \id_{x})$$
But then:
$$\phi_j \otimes \id_{x} \in \End_{\cC}(n_j Y_j \otimes x)\cong \bigoplus\limits_{i} \End_{\cC}( p_{i,j} n_j Z_i)$$
