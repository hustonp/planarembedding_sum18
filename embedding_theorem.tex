...
\begin{proof}
 ...
 For some nonnegative integers $n_1,\ldots n_r$ so then we have:
	\[\End_{\cC}\left(m\otimes\xalt\right)\cong \bigoplus\limits_{j}\End_{\cC}\left(n_j Y^l_j\right) \]
...
Recall that:
$$A_n=\End_{\cC}\left(m\otimes\xalt\right)\cong \bigoplus\limits_{j}\End_{\cC}\left(n_j Y_j\right)$$
if we have that 
$$m \otimes \xalt \cong \bigoplus\limits_{j}\left(n_jY_j\right)$$
...
where each $\phi_j \in \End_{\cC}\left(n_jY_j\right)$. Then, distributing tensor products over the sum, we may write $\phi$ as 
$$\bigoplus_{j}\left(\phi_j \otimes \id_{x}\right)$$
But then:
$$\phi_j \otimes \id_{x} \in \End_{\cC}\left(n_j Y_j \otimes x\right)\cong \bigoplus\limits_{i} \End_{\cC}\left( p_{i,j} n_j Z_i\right)$$
