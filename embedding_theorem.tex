...
\des{Do we really want the discussion of the planar algebra and this lemma here? It seems out of place given that we don't define the PA for strongly Markov Inclusion until the next section.}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{cor}
\label{cor:OntoCor}
If $x \in A'_k \cap A_{n+k}\subset A_0'\cap A_{n+k}$,
then there is a unique $y\in A_0'\cap A_n$ such that
...
\end{tikzpicture}\,.
\]
Hence adding $k$ strands to the left is a unital $*$-algebra isomorphism $A_0'\cap A_n \to A_k'\cap A_{n+k}$.
%and we have a similar result for elements of $A_3'\cap A_{n+3}$ when represented in string diagrams in $A'_{1}\cap A_{n+3}$.%
\end{cor}
\des{This is false as stated, in general $A'_0\cap A_n$ is not isomorphic to $A'_1\cap A_{n+1}$. I can't immediately see where the proof goes wrong though. Also I believe we need to state it such that it also gives an isomorphism for the negative box spaces.}
\begin{proof} 
