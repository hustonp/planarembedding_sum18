...
The proof in \cite{1810.06076} is mostly in the language of tensor and module categories, for which a canonical reference is \cite{MR3242743}.
...
By induction, the Bratteli diagram for $M_k\subseteq M_{k+1}$ is isomorphic to a full subgraph of the principal graph, with the isomorphism uniquely determined by the reflections.
...
\begin{ex}
 ...
 The algebra and Markov tower structure are as in \cite{MR2679382}:
...
\begin{ex}
 As a special case of the above, % HERE consider citation
 the Temperley-Lieb-Jones algebras $TLJ(d)_n$ of modulus $d\geq 2$ with the usual Jones projections and Markov traces form a Markov tower with principal graph $A_{\infty}$; when $d=4\cos(\pi/n)^2$, the principal graph is $A_{n-2}$. % HERE check
 The algebra $TLJ(d)_n$ is the $2n$-th box space of the Temperley-Lieb subfactor planar algebra of \cite{math.QA/9909027} with loop parameter $d$.
 It is spanned as a complex vectorspace by (unshaded) planar tangles with no input discs, which we depict as tickets cotaining $n$ strings, with $n$ endpoints on the bottom of the ticket and $n$ on the top, using the distinguised interval on the boundary of a tangle as the left side of the ticket.
 Composition is accomplished by stacking tickets and replacing any closed loops with a factor of $d$. % HERE make sure we haven't mixed up d with d^2 or d^\frac{1}{2} anywhere.
 As before, inclusion involves adding a string on the right, and conditional expectation involves closing off the rightmost string.

 Since the Jones projections generate each $TLJ(d)_n$ as an algebra, there is a canonical homomorphism $TLJ(d)_\bullet\to M_\bullet$ for any Markov tower $M_\bullet$ of index $d$.
 % HERE and consider one more sentance about how this carries over to the other two perspectives considered in this paper.
\end{ex}
...
\in M_{2k+3}.
\end{align*}
 The principal graph of $M_\bullet$ is precisely the fusion graph of $\cM$ with respect to $X$: vertices are isomorphism classes of simple objects in $\cM$, and there are $\dim(\cM(YX,Z))$ edges from $Y$ to $Z$ (where $Y$ is unshaded on the right).
\end{ex}
...
\begin{cor}
\label{cor:OntoCor}
Adding $k$ strings to the left of $x$ gives unital $\ast$-algebra isomorphisms
...
For $x\in A'_k\cap A_{n+k}$, the preimage $y\in A_0'\cap A_n$ is uniquely determined by
\begin{equation*}
%\begin{tikzpicture}[baseline]
%\draw (0,-.7) -- (0,.7);
%\roundNbox{unshaded}{(0,0)}{.3}{0}{0}{$x$};
