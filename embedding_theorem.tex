...
We call the tower of algebras with Jones projections $(M_n,\tr_n, e_n)$ a \emph{Markov tower}, as it satisfies the following properties:
\begin{enumerate}[(1)]
\item
...
Since $\cM$ is finitely semisimple, the Markov tower $(M_n, \tr_n, e_n)$ has \emph{finite depth}, meaning the principal graph is finite.
...
\begin{defn}
The \emph{principal graph} of the Markov tower $(M_n,\tr_n)$ with Jones projections $(e_n)$ consists of the \emph{new} vertices at every level $n$ of the Bratteli diagram, together with all the edges connecting them.

A Markov tower is said to have \emph{finite depth} if the principal graph is finite.
\end{defn}


It follows that a Markov tower has finite depth if and only if there is $n\in \bbN$ such that $Y_n = (0)$, as in \ref{prop:ElementaryMarkov} \eqref{EP:FiniteDepth}. 
...
\item[\underline{Proof of (2):}]
Given that there exists a Pimsner-Popa basis for $pA_1p$ over $pA_0p$ by (1), the inclusion is Markov if and only if the Watatani index \cite{MR996807} is a scalar by \cite[1.1.4(c)]{MR1278111}.
We now calculate
$$
\sum_{pbap} pbap(pbap)^*
=
...
Given a Markov tower $(M_n,\tr_n, e_n)$, no additional assumptions on $p\in P(M_0)\setminus\{0\}$ are necessary to obtain another Markov tower by compression.

\begin{prop}
Suppose $(M_n,\tr_n, e_n)$ is a Markov tower of finite dimensional von Neumann algebras and $p\in P(M_0)$ is a nonzero projection.
Then $(pM_np, \tr_n^p, pe_n)$ is a Markov tower.
\end{prop}
\begin{proof}
...
That $pe_n$ implements the trace-preserving conditional expectation $pM_np \to pM_{n-1}p$ follows from \eqref{eq:CompressionImplementsConditionalExpectation}, which holds for an arbitrary inclusion of tracial von Neumann algebras.
This immediately implies that $E_{n+1}^p(pe_n) = pE_{n+1}(e_n) = d^{-2}p = d^{-2} 1_{M_n}$.
...
\nn{from earlier. find me a home:
\\
\\
and there is a canonical planar algebra structure on this tower, called the \textit{canonical planar $\ast$-algebra of a strongly Markov inclusion} corresponding to $A_0\subseteq (A_1,\tr_1)$. One important feature of this construction is that it depends on the existence of a Pimsner-Popa basis for $A_1$ over $A_0$. 
...
Planar algebras were originally introduced in \cite{math.QA/9909027}. We will use the same definitions and conventions as \cite{MR2812459}, \cite{MR2679382}, \cite{jones}, and \cite{planaralg2}. The reader is referred to these references for further information on planar algebras. From any subfactor planar algebra, one may construct a 2-category. We refer the reader to \cite{paragroup} and \cite{paragroup2} for more details. We adopt the definitions used in \cite{paragroup}, since we wish to work with shaded planar algebras.
...
\nn{I expect the above theorem trivially extends to an equivalence of categories, as follows.
I claim morphisms of cyclic module categories and cyclic planar modules are automatically isomorphisms.
Hence the above categories truncate to (are contractible to) \emph{sets}.
What is more interesting is the following theorem, which tells us when the underlying $\cC$-module $\Cstar$ categories are dagger equivalent.}
