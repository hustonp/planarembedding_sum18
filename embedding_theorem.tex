...
\end{defn}
Due to \cite{MR996807} we have the following:
\begin{prop}
...
\begin{defn}
Let $\Tr_2$ be the canonical, faithful, normal, semifinite trace for $A_2$. The inclusion is said to be \textit{strongly Markov} if:
\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Shifts and compressions} 

\nn{prove shifting is an iso on relative commutants}\\
%%%%%%%%%%%%%Shifting%%%%%%%%%%%%%
Suppose that $(A_n,\tr_n)$ is a strongly Markov sequence of von Neumann algebras. We know from \cite{MR2812459} that the inclusion $A_0 \subseteq A_2 \subseteq \left(A_4, \tr_4,d^{-2}(e_1e_2)(e_3e_2)\right)$ is standard. Thus we can find a Pimsner-Popa basis $B$ for $A_2$ over $A_0$.
\begin{remark}
The commutant conditonal expectation $E^{A'_0}_{A'_2}$ is given by:
\[
 E^{A'_0}_{A'_2}(x)\quad=\quad d^{-4} \sum_{b \in B} bxb^{\ast} \quad=\quad 
d^{-2}
...
\node at (0,0) {\scriptsize{$x$}};
\node at (0,-.9) {\tiny{$n-2$}};
\node at (-1.05,0) {\tiny{$d^{-2}$}};
\end{tikzpicture}
\]
\end{cor}
Let us denote the relative commutant algebra on the inclusion $A_0 \subseteq A_1$ by $\mathcal{A}_{\bullet}$ and the relative commutant algebra on $A_2 \subset A_3$ by $\mathcal{B}_{\bullet}$.
\begin{lem}[Shift by 2 Isomorphism]
...
\end{tikzpicture}
\]
is a planar $\ast$-algebra isomorphism.
\end{lem}
\begin{proof}
...
\begin{thm}
\label{thm:CompressionIsomorphism}
Suppose $M_0\subset M_1$ is a strongly Markov inclusion of finite von Neumann algebras and $p\in P(M_0)$ is a nonzero projection.
% with central support $1$.
Then the inclusion $pM_0p \subset pM_1p$ is strongly Markov.
\end{thm}
\begin{proof}
\nn{TODO.
\\
\\
Will also require that $M_0pM_0 = M_0$, which is stronger than central support $1$ so that
$$
pM_1p (pe_1)pM_1p
=
pM_1M_0pe_1M_0M_1p
...
Then $(pM_np, \tr_n(p\, \cdot\,)/\tr_n(p), pe_n)$ is a Markov sequence.
\end{cor}
\begin{proof}
\nn{todo: should be OK.}
\end{proof}
...
The canonical planar $\ast$-algebra of a strongly Markov inclusion $A_0\subseteq (A_1,\tr_1)$ is defined as follows:
	...
	\]
	Both of these maps are well-defined, by \cite[Lem2.29]{MR2812459}.
...
\begin{thm}[Penneys, Jones, \cite{MR2812459}] \label{planaralgebraisomorphism}
...
The following two sections will describe two isomorphisms between planar algebras constructed in this manner both of which are well known to experts and will motivate constructions detailed in the later sections of this article.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Shift and compression isomorphisms of planar algebras} 
...
Suppose $M_0\subset M_1$ is a strongly Markov inclusion of finite von Neumann algebras and $p\in P(M_0)$ is a nonzero projection with central support $1$.
Then 
%\begin{enumerate}
%\item
...
%\item
%\nn{rephrase...}
the map $x\mapsto px$ from $M_0'\cap M_n \to pM_0'\cap pM_np \cong p(M_0'\cap M_n)$
\nn{definition on $\cP_{n,-}$ too}
is planar $\dag$-algebra isomorphism.
%\end{enumerate}
\end{thm}
\begin{proof}
We prove the maps $\Phi_{n,\pm}$ satisfy the conditions of Lemma \ref{lem:SufficientConditionsForPlanarMap}.

First, note that $\Phi_{n,\pm}(e_n) = pe_n$, so Jones projections in $\cP_\bullet$ map to Jones projections in $\cQ_\bullet$ by \nn{earlier section}.
\nn{TODO: edit below}
