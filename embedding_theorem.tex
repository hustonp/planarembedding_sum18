...
We can therefore view objects $\cG$ as the $2$-category of bimodules over the algebra objects $1_{0,+}$ and $1_{0,-}$ and view $\cM$ as the category of $1_{0,+}-A$ and $1_{0,-}-A$ bimodules. 
...
Recall that:
$$A_n=\End_{\cC}(m\otimes\xalt)\cong \End_{\cC}(\bigoplus\limits_{j}n_j Y_j)$$
if we have that 
...
But then:
$$\phi_j \otimes \id_{x} \in \End_{\cC}(n_j Y_j \otimes x)\cong \End_{\cC}(\bigoplus\limits_{i} p_{i,j} n_j Z_i)$$
...
{\footnotesize{
\bibliography{EmbeddingBibliography}
}}
\end{document}
