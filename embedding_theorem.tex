...
\end{enumerate}
Here, we use the definition of $\tr^p_n$ from \eqref{eq:CompressedTrace1}
...
We already saw condition (1) from the lemma holds in \eqref{eq:CompressionImplementsConditionalExpectation1}.
To see condition (2) from the lemma holds, note that $E^p_2(pxp) = pE_2(x)p$ for all $x\in A_2$ as in  \eqref{eq:CompressedConditionalExpectation1}.
...
\begin{cor}
\nn{pivotal Tempreley-Lieb-Jones modules classified by graphs with \emph{vertex} weighting rather than edge weighting following DeCommer-Yamashita.}
\end{cor}
