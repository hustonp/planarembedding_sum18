...
=
\Tr^\cM_{m}(
(\id_m \lhd\coev_c^\dag) \circ (f\lhd\id_{\overline{c}}) \circ (\id_m \vartriangleleft \coev_c)
),
$$
...
To see that the $\Tr_k$ give a positive module trace, we reduce the general case to the case of endomorphisms as follows: suppose $f\in\cM([n]\to[n+2k])$. Obtain $g\in\C(1\to(X^?)^{\alttens[2k]})$ from $1\in\cP_{k,\pm}$. Then \[\Tr_n(f^\dag\circ f)=\Tr_{n+2k}((f\lhd g)^\dag\circ(f\lhd g))\Tr_{2k}(g^\dag\circ g)^{-1}=d^k\Tr_{n+2k}((f\lhd g)^\dag\circ(f\lhd g))\]
...
\end{prop}
	...
	From equation \ref{eq:LinkingAlgebra}, it is clear that $\pi$ is injective, unital, $\bbC$-linear, and respects the $\dag$-structure. The difficulty in seeing that $\pi$ is an algebra homomorphism is in seeing that it respects multiplication. Notice however that because $\pi$ respects $\dag$ we only need to check 3 types of composition:
	\begin{enumerate}
