...
for the $\rm C^*$ setting, see the textbook \cite{MR3204665}.
...
\end{lem}
\begin{proof}
Recall from \cite[Prop.~2.25]{} we have an isomorphism of $A_1-A_1$ bimodules
$$
\theta_k : 
...
we isotope it into a \emph{standard form} \nn{} and read from bottom to top.
Under the isomorphism $\theta_k$. we have
$$
d^{-k}\,
\begin{tikzpicture}[baseline=-.1cm]
\draw (-.6,-.7) -- (-.6,.7) node[above]{$\scriptstyle k$};
...
\draw (.3,.3) -- (.3,.7) node[above]{$\scriptstyle k$};
\draw (-.6,.7) -- (-.6,-.1) arc (-180:-90:.6cm) node[above]{$\scriptstyle k$} arc (-90:0:.6cm) -- (.6,.7);
...
$$
Here, we define $B:= \set{c_k \otimes \cdots \otimes c_1}{c_1,\dots,c_k\in C}$, which is easily seen to be a Pimsner-Popa basis for $A_k$ over $A_0$ under the $A_0-A_0$ bimodule isomorphism $A_k \cong \bigotimes^k_{A_0} A_1$.
(Observe this isomorphism )
\todo{fix all this.}
Next, we observe that the multiplication is given graphically by stacking, which under standard form isotopy looks as follows:
$$
\begin{tikzpicture}[baseline=-.1cm]
\draw (-.6,-.7) -- (-.6,.7) node[above]{$\scriptstyle k$};
\draw (-.15,-.7) -- (-.15,.7) node[above]{$\scriptstyle k$};
...
%
%In the figure below, we do the special case $k=3$, where $C$ is some choice of Pimsner-Popa basis for $A_1$ over $A_0$:
%$$
%\begin{tikzpicture}[baseline=-.1cm]
%\draw (-.15,.3) arc(0:180:.15) -- (-.45,-.3) arc(-180:0:.15);
%\draw (-.75,-.7) -- (-.75,.7);
...
%\begin{tikzpicture}[baseline=-.1cm]
%\draw (0,0) -- (0,.5);
%\roundNbox{unshaded}{(0,0)}{.3}{.2}{.2}{$x$};
%\end{tikzpicture}
%\todo{}
%$$
%$$
%B=\set{c_{k-1} v_{k-1}c_{k-2}v_{k-2}\cdots v_{1} c_1}{c_1,\dots, c_{k-1} \in C}
%$$
%for some/any Pimsner-Popa basis $C$ for $A_1$ over $A_0$ 
%and
%$v_i := e_{i}\cdots e_2e_1$ for all $i$.
%\todo{something like this works... check the indices}
\end{proof}
