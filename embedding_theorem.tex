...
Suppose $(\cM, \Tr^\cM, m)$ is an indecomposable pivotal right $\cC$-module $\Cstar$ category, and $m\in \cM$ is a distinguished simple object with $m= m \vartriangleleft 1_0$.
	We build a connected right planar $\cP_\bullet$-module $\cM_\bullet$ by defining $\cM_k := \End_\cC( m \vartriangleleft X^{\text{alt}\otimes k})$ for $k\geq 0$, and we define the action of the shaded planar module operad via the diagrammatic calculus for $\cM$. The process is similar to that in \cite[Def.~3.2]{MR2812459}: we first define a standard form for tanges of the shaded planar module operad, such that every tangle is isotopic to one in standard form. We say a tangle is in \textit{standard form} if 
	\begin{enumerate}
	...
	Suppose $T$ is a tangle in standard form with input semidiscs of types $(n_i)_{i=1}^r$, input discs of types $((m_j,\pm_j))_{j=1}^s$, and output type $n$. Choose endomorphisms $\xi_i\in\cM_{n_i}$ and $\eta_j\in\End((x^{?_j})^{\alttens{m_j}}$. 
	As in \cite{MR2812459}, we intend to build up $T((\xi_i),(\eta_j))$ by sliding a horizontal line from the bottom of $T$ to the top. 
	For each $h\in[0,1]$, we will choose a $\tau_h$ in some $\cM_\bullet$, imagining $\tau_h$ as the value corresponding to a horizontal line through $T$ at height $h$, where $0$ represents the bottom of $T$ and $1$ represents the top. When the line at $h$ does not touch extrema of strands, $\tau_h\in\cM_{w(h)}$, where $w(h)$ is the sum of the number of strands and the index of the box space of the input (semi)disc (if any) at height $h$. 
	While the line of height $h$ does not pass through extrema or the top of an input (semi)disc, $\tau_h$ remains constant. 
	The very bottom of $T$ looks the same as the identity tangle, so we set $\tau_0=1\in\End(mx^{\alttens{n}})$; by the top, we hope to define $T((\xi_i),(\eta_j))=\tau_1$. 
	\nn{Add a representative tangle-in-process around here.}

	It remains to describe what happens when the horizontal line passes an etremum or the top of an input (semi)disc. If $\tau_h\in\cM_k$ and between $h$ and $h'$, the horizontal line passes only the top of the $i$-th input semidisc, we simply set $\tau_{h'}=\xi_i\circ\tau_h$. Of course, in general $n_i<w(h)$; then we right-include $\xi_i$ into $\cM_{w(h)}$ by $\xi_i\to\xi_i\otimes1_{x^{\alttens{w(h)-n_i}}}$. Simiarly, when passing through the top of an input disc, we include $\eta_j$ on the right and the left before compsing, where left inclusion sends $\eta_j\to 1_{mx^{\alttens{?}}}\otimes\eta_j$. Passing through the maximum of a string becomes compsing with the coevalutation $\coev_{x}$ or $\coev_{\overline{x}}$, depending on the shading, inlucded on the left and right in the same way; similarly, passing through the minimum of a string becomes composing with a suitable evaluation. 
...
So 
$$
\pi(y\circ x)=d^{-2i-j-k}E_{n+i+j}(yx \cdot \TLPlusPIso{n}{i}{j}) \cdot \TLCapJoin{n}{i}{j}{j}{i} \cdot \TLTStart\TLTThrough{n}\TLTThrough{2i}\TLTThrough{2j}\TLTCapcup{k}\TLTEnd
$$ 
But then Lemma \ref{lem:LeftKink} immediately gives that:
$$
\pi(y\circ x)=d^{-i-j-k} yx \cdot \TLTStart\TLTThrough{n}\TLTCapcup{i}\TLTCapcup{j}\TLTThrough{2k}\TLTEnd \cdot \TLTStart\TLTThrough{n}\TLTThrough{2i}\TLTThrough{2j}\TLTCapcup{k}\TLTEnd = \pi(y) \cdot \pi(x)
$$
...
Then 
$$
\pi(y\circ x)=d^{-2i-j-k}E_{n+i}(yx \TLMinusPIso{n}{i}{j})\TLTStart\TLTThrough{n}\TLTCapcup{i}\TLTCapcup{j}\TLTCapcup{k}\TLTEnd
$$ 
First observe that using lemma \ref{lem:LeftKink} we have:
$$
d^{-i}E_{n+i}(E_{n+2i}(yx\TLMinusPIso{n}{i}{j}))\TLTStart\TLTThrough{n}\TLTCapcup{i}\TLTEnd= (E_{n+2i}(yx\TLMinusPIso{n}{i}{j})\TLTStart\TLTThrough{n}\TLTCapcup{i}\TLTEnd
$$
And since $E_{n+i}=E_{n+i}\circ E_{n+2i}$ we have that:

$$
\pi(y\circ x)= d^{-i-j-k}E_{n+2i}(yx\TLMinusPIso{n}{i}{j})\TLTStart\TLTThrough{n}\TLTCapcup{i}\TLTCapcup{j}\TLTCapcup{k}\TLTEnd
$$
Now observe that $yx\TLMinusPIso{n}{i}{j}\in M_{n+2i+j}$ and we know from proposition \ref{prop:MultistepJonesProjections} we know that $\TLTStart\TLTThrough{n}\TLTThrough{2i}\TLTCapcup{j}\TLTEnd$ implements $E^{n+2i+j}_{n+2i}$ so:

$$
\pi(y\circ x)=d^{-i-j-k}\TLTStart\TLTThrough{n}\TLTThrough{2i}\TLTCapcup{j}\TLTThrough{2k}\TLTEnd \cdot yx \cdot \TLMinusPIso{n}{i}{j} \cdot \TLTStart\TLTThrough{n}\TLTCapcup{i}\TLTCapcup{j}\TLTCapcup{k}\TLTEnd = \pi(y) \cdot \pi(x)
$$

\eqref{downUP} Let $x:[n]\rightarrow [n+2i+2j]$ and $y:[n+2i+2j]\rightarrow [n+2i]$
Then:
$$
\pi(y\circ x)=d^{-i-j-k}y\cdot \TLPlusPIso{n}{i}{j} \cdot x \cdot \TLTStart\TLTThrough{n}\TLTThrough{2i}\TLTCapcup{j}\TLTCapcup{k}\TLTEnd
$$
and
$$
\pi(y)\cdot \pi(x)= d^{-i-j-k}y\cdot  \TLCapJoin{n}{i}{j}{j}{i} \cdot x \TLTStart\TLTThrough{n}\TLTThrough{2i}\TLTThrough{2j}\TLTCapcup{k}\TLTEnd
$$
but these are equal as:
$$
y\cdot \TLPlusPIso{n}{i}{j} \cdot x \cdot \TLTStart\TLTThrough{n}\TLTThrough{2i}\TLTCapcup{j}\TLTCapcup{k}\TLTEnd= y\cdot \TLPlusPIso{n}{i}{j} \cdot\TLTStart\TLTThrough{n}\TLTThrough{2i}\TLTCapcup{j}\TLTThrough{k}\TLTEnd \cdot x \cdot  \TLTStart\TLTThrough{n}\TLTThrough{2i}\TLTThrough{2j}\TLTCapcup{k}\TLTEnd = y\cdot  \TLCapJoin{n}{i}{j}{j}{i} \cdot x \cdot \TLTStart\TLTThrough{n}\TLTThrough{2i}\TLTThrough{2j}\TLTCapcup{k}\TLTEnd 
$$
...
\label{def:ModuleFromMarkovTower}
	If $M_\bullet$ is a Markov tower with modulus $d$, the corresponding $\cT\cL\cJ(d)$-module is just the category $\cM$ of projections of $M_\bullet$, as in definition \ref{def:MarkovProjections}. By construction, to define an action of $\TLJ$ on $\cM$, it suffices to define the action of the objects $\TLJ_m$ on $1_{[n]}$ for every $n$ and $m$, and then define the action functorially on morphisms. 
	
	We set $[n]\vartriangleleft\TLJ_m=[n+m]$. As for morphisms, we first consider the case where one morphism is the identity. 
	If $g\in\TLJ_{a\to b}$, we simply add $n$ strings on the left to obtain $1_{M_n}\vartriangleleft g\in\TLJ_{n+a\to n+b}\subseteq M_{n+a\to n+b}$. % maybe change this since the map $\TLJ\to M_\bullet$ is not an inclusion
	...
	Let $1\cdot g$ be the morphism obtained from letting $g$ act on one, this time considering $g$ as an element of $\TLJ_{2k+j+i}$. By definition, we have 
	\begin{align*}
		(f\vartriangleleft 1)\circ(1\vartriangleleft g) &= E^{n+j+4k+2i}_{n+j+3k+i}\left(f\circ\TLPlusPIso{n}{k}{j+2k+i}\circ (1\cdot g)\circ\TLPlusPIso{n+j+k}{k+i}{k}\right)\\
		&= f\circ E^{n+j+3k+2i}_{n+j+2k+i}\left(\TLPlusPIso{n}{k}{j+2k+i}\circ (1\cdot g)\circ\TLPlusPIso{n+j+k}{k+i}{k}\right)\\
		\intertext{But $g$ is a morphism in Temperley-Lieb, and hence so is $1\cdot g$, allowing us to pull $1\cdot g$ through the partial isometry: }
		&= (1\cdot g)\circ f\circ E^{n+j+3k+2i}_{n+j+2k+i}\left(\TLPlusPIso{n}{k}{j+2k+i}\circ\TLPlusPIso{n+j+k}{k+i}{k}\right)\\
		&= (1\cdot g)\circ f\circ E^{n+j+3k+2i}_{n+j+2k+i}\left(\TLPlusPIso{n}{k}{j}\circ\TLPlusPIso{n+j+k}{k}{i+k}\right)\\
		&=(1\vartriangleleft g)\circ(f\vartriangleleft 1)
	\end{align*}

	From here, it is routine to verify that defining $f\vartriangleleft g=(f\vartriangleleft1)\circ(1\vartriangleleft g)$ makes $\cdot\vartriangleleft\cdot$ a bifunctor. 
	Finally, we check that the action is associative.
	\nn{Check that too. Or wave it off, if similar to the previous stuff.}

	It is straightforward to verify that the trace on $M_\bullet$ \nn{normalized?} makes $M_\bullet$ a pivotal right module. 
\end{defn}
...
\end{thm}
\begin{proof}
The map is an isomorphism between box spaces due to \ref{cor:OntoCor}. In order to show that this map commutes with the action of tangles, we just have to show that it satistfies the requirements of \ref{lem:SufficientConditionsForPlanarMap}. We draw the string diagrams of $\cB_{\bullet}$ in red in order to increase clarity.
\begin{enumerate}
\item (Right Inclusion)
\[
\begin{tikzpicture}[baseline=-0.1]
\draw[red] (0,-0.7) -- (0,0.7);
\draw[red] (0.45,-0.7) -- (0.45,0.7);
...
\fill[ctwoshading] (0,-0.7) rectangle (0.45,0.7);
\draw[red] (0,-0.7) -- (0,0.7);
\draw[red] (0.45,-0.7) -- (0.45,0.7);
\roundNbox{unshaded, draw=red}{(0,0)}{.3}{0}{0}{\textcolor{red}{\tiny{$\Phi(x)$}}};
\node at (0,-0.9){\textcolor{red}{\tiny{$n$}}};
...
\end{lem}
In this case, where $B$ is isomorphic to the basic construction $A_2$ of $A_0\subseteq A_1$, we call the inclusion $A_0\subseteq A_1\subseteq B$ \textit{standard}, after \cite{penneys}. 
