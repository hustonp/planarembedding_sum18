...
\begin{tikzpicture}[baseline]
	...
	\node at (0,-0.9){\tiny{$2r$}};
\end{tikzpicture} \]
This map sends $Q_{n , +}$ to $P_{n,+}$ and $Q_{n , -}$ to $P_{n,-}$. In order to check that $\Phi$ is a planar-$\ast$ algebra map, we use Lemma 2.48 in \cite{penneys}.
We shall state it here for the reader's convenience:
\begin{lem}
\label{lem:SufficientConditionsForPlanarMap}
Suppose $\Phi_{n,\pm} : \cP_{n,\pm} \to \cQ_{n,\pm}$ is a collection of unital $*$-algebra maps such that
\begin{enumerate}[(1)]
\item
...
$$
\end{enumerate}
Then $\Phi$ is a map of planar $*$-algebras.
\end{lem}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Compressing a strongly Markov inclusion}
...
The following lemma adapted from \cite{MR2812459} provides sufficient conditions for a collection of maps to be a morphism of subfactor planar algebras.
\todo{The following is an extra copy of Lemma 5.1}
\begin{lem}
\label{lem:SufficientConditionsForPlanarMap}
Suppose $\Phi_{n,\pm} : \cP_{n,\pm} \to \cQ_{n,\pm}$ is a collection of unital $*$-algebra maps such that
\begin{enumerate}[(1)]
...
\end{lem}
\begin{proof}
This follows from the proof of \cite[Lemma 2.49]{MR2812459}.
\end{proof}
