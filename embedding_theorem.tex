...
\begin{remark}
Notice that shifting a Markov tower simply truncates the Bratelli tower (as defined in \ref{Rem:BratelliTower}) and by a similar argument to the on used to show \ref{Fact:BratteliPrincipal} the Principal graph is unchanged.
\end{remark}
...
\begin{remark}
If we compress the Markov tower $(M_n,\tr_n, e_{n+1})$ by $p$ then we can determine Bratelli tower for $(pM_np, \tr_n^p, pe_{n+1})$ (and thus the principal graph). First, if $p$ has central support $1$ then the Bratelli tower is in fact unchanged. If $p$ does not have central support $1$ then any vertices (and edges emanating from them) corresponding to simple summands of $M_0$ where $p$ does not have support disappear, all other vertices and edges are unchanged. Then vertices corresponding to simple summands $M_1$ may have no edges coming in from the previous level, these vertices and their adjacent edges are then removed. Continuing in this fashion we can then read up the Bratelli tower for  $(M_n,\tr_n, e_{n+1})$ to obtain the Bratelli tower for $(pM_np, \tr_n^p, pe_{n+1})$.
\end{remark}
...
%Let $\Gamma$ be the principal graph of  $\left( M_n, \tr_n, e_{n} \right )$ and $\Gamma '$ be the principal graph of  $\left( M_{kn}, \tr_{kn}, e_{kn} \right )$. 
Then the principal graph for $\left( M_{kn}, \tr_{kn}, e_{kn} \right )_{n\geq 0}$ can be read off the Bratelli Tower as follows. For $n\geq 0$ we can consider the section of Bratelli tower of $\left( M_n, \tr_n, e_{n+1} \right )_{n \geq 0}$ corresponding to the inclusions: $M_{kn}\subseteq M_{kn+1}\subseteq \ldots \subseteq M_{k(n+1)}$. This will define the Bratelli diagram for $M_{kn}\subseteq M_{k(n+1)} $. Because the Bratelli diagram for $M_{\ell+1}\subseteq M_{\ell+2}$, for each $\ell$, is given by the upwards reflection of the Bratelli diagram for $M_{\ell}\subseteq M_{\ell+1}$, with some new vertices, the section of the Bratelli tower corresponding to $M_{k(n+1)}\subseteq M_{k(n+1)+1}\subseteq \ldots \subseteq M_{k(n+2)}$ contains a reflection of the section of the Bratelli tower corresponding to $M_{kn}\subseteq M_{kn+1}\subseteq \ldots \subseteq M_{k(n+1)}$, and each path of length $k$ beginning at a vertex corresponding to some simple summand of $Y_{k(n+1)}$ and ending at a vertex of $Y_{k(n+2)}$ gives an edge in the principal graph of $\left( M_{kn}, \tr_{kn}, g_{n+1} \right )_{n\geq 0}$.
