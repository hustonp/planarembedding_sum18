...
By Example \ref{ex:ShiftMarkovTower}, we may assume $j=0$.
...
\end{prop}
\begin{proof}
\nn{Use Lemma \ref{lem:MultistepJonesProjections}}
\end{proof}
...
The following definition of planar module is more general than the annular planar modules introduced in \cite{MR1929335}. Our planar modules will correspond to the above notion of module over a multitensor category, whereas annular planar possess additional structure. \nn{Add a citation for details.}
\begin{defn}
	An \textit{annular tangle} is a tangle of the planar operad with a choice of distinguished internal disk. This notion first appears in \cite{MR1929335}. % clumsy parallelism
\end{defn}
\begin{defn}
	A \textit{stationary tangle} \nn{as opposed to rotating. Bikeshed this.}
	is an annular tangle for which the * for the distinguished internal disk and the * on the output disk are on the same region, i.e. they are not separated by any strands. 
\end{defn}
\begin{defn}
	\nn{In our cyclic planar modules, the box spaces that are (un)shaded on the left are simply $0$, depending on the choice of strand. Maybe mention this, perhaps in the example?}
	Suppose $\cA_\bullet$ is a planar algebra. A \textit{right planar module} $\cM_\bullet$ over $\cA_\bullet$ is a collection of vectorspaces $(M_{n})_{n=0}^\infty$ endowed with an action of $\cA_\bullet$:
	for every stationary tangle $T$ of type in the shaded planar operad, we have an action $\alpha_T:\cM_{k_0,\pm_0}\otimes(\otimes_{i=1}^r\cA_{k_i,\pm_i})\to\cM_{n,\pm}$, where $(k_0,\pm_0)$ is the type of the distinguished disk, $(n,\pm)$ is the type of the output disk, and $(k_i,\pm_i)_{i=1}^r$ are the types of the remaining input disks. The several $\alpha_T$ are required to be compatible with the operadic composition of tangles, as in the definition of a planar algebra. 
\end{defn}
Note that plugging a stationary tangle into the distinguished input disk of a stationary tangle yields a stationary tangle, as does plugging an arbitrary tangle into a non-distinguished input disk. 

The definitions of stationary tangles and right planar module are motivated by the diagrammatic calculus of the paragroup. We may represent depict a stationary tangle by placing the tangle in standard form with the distinguished disk at the bottom, and expanding the boundaries of the distinguished input disk and output disk to parallel lines, in effect sending a curve connecting the * of the distinguished disk and the * on the output disk to infinity. Because the tangle is stationary, the distinguished disk need not cross any strands to reach the bottom of the standard form, and the curve sent to infinity can also avoid crossing strands or input disks. In this diagrammatic form, a stationary tangle looks just like a morphism between $1_{\cM_{k_0,\pm_0}}$ and $1_{\cM_{n,\pm}}$ in the paragroup, and indeed, the action $\alpha_T$ gives a map $\otimes_{i=1}^r\cA_{k_i,\pm_i}\to\Hom(\cM_{k_0,\pm_0},\cM_{n,\pm})$, where the latter $\Hom$-space is $\Hom$ in a module category over the paragroup. % extremely awk 
\nn{todo: add an example of a this diagrammatic transformation}
...
Then for any $\pm$ vertex $v$ of $\Gamma$, we get a cyclic right planar module $\cM_\bullet = \cM(\Gamma,v)_\bullet$ by defining $\cM(\Gamma, v)_{n}:= p_v \cG_{n,\pm}$ and action of the planar module operad by
\nn{todo}

Now if $\cQ_\bullet$ is a finite depth subfactor planar algebra and $\Phi : \cQ_\bullet \hookrightarrow \cG_\bullet$ is a planar algebra embedding, then we may endow $\cM_\bullet$ with the structure of a cyclic right planar $\cQ_\bullet$ module by defining 
$$
...
is a Markov tower $M_\bullet = (M_n, \tr_n, p_{n+1})$
together with for every $n\geq 0$,
a family of unital $*$-algebra homomorphisms $\rho^n_\bullet : Q_\bullet \to M_\bullet$ where
\begin{align*}
\rho^n_{2k} : Q_{n,+} \to M_n'\cap M_{2k+n}
\qquad
\qquad
\rho^n_{2k+1} : Q_{n,-} \to M_n'\cap M_{2k+n+1}
\end{align*}
