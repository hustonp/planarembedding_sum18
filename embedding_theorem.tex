...
Let $x\in \cC_{01}$ be the strand as pictured below:
...
By the construction of $\cC$ $x$ is a generating object for $\cC$. Now if we take any $m\in \cC_{01}$ then we have that any object of $\cM$ is isomorphic to a direct summand of $m\otimes \xalt$ for some nonnegative integer $n$, where $$
...
We will construct a finite depth Markov sequence of algebras from the action of $\cC$ on $\cM$ as follows. Let $A_n=\End_{\cC}(m\otimes \xalt)$, $A_n$ is an algebra when multiplication is defined to be composition of morphisms. We can represent objects in $\End_{\cC}(m\otimes \xalt)$ as: 
$$
...
\end{tikzpicture}
$$
where the red strand represents $m$ and the $n$ represents $\xalt$. We have a faithful tracial state $\tr_n:A_n\to \C$ given by
...
\begin{proof}
First recall that for a finite depth Markov sequence the principal graph is canonically isomorphic to the Bratelli for the inclusion $A_n\subset A_{n+1}$ when $n$ is such that this inclusion is now standard. So it suffices to show this graph is isomorphic to the fusion graph for $\cM$. 

Let $s^i_1,\ldots s^i_{r_i}$, for $i=0$ or $1$ be a collection of nonisomorphic simple objects in $\cC_{2i}$ such that every object in $\cC_{2i}$ is isomorphic to a direct sum of these elements, as in the previous proof. We will work the index $i$ mod 1 so that we may write $s^{i+1}_j$ for brevity of notation. Let the nonnegative integers $p^{i,j}_k$ be defined by the following equation:

$$s^i_j\otimes x^{?}=\bigoplus\limits_{k=1}^{r_{i+1}}p^{i,j}_ks^{i+1}_k$$

Where $x^{?}$ is $x$ if $n$ is even and $\overline{x}$ if $n$ is odd.  These coefficients are exactly the coefficients for the adjacency matrix of the fusion graph for $x^{?}$.
...
Furthermore the inclusion of $A_n \hookrightarrow A_{n+1}$ is a unital inclusion and it's given by $\phi \mapsto \phi \otimes \id_{x^{?}}$. Each $\phi \in A_n$ is uniquely defined a direct sum:
$$\bigoplus\limits_{j=1}^{r_i}\phi_j$$
Where each $\phi_j$ is an element of $\End_{\cC}(n_js^{i}_j)$. Then by the commutativity of tensor products and direct sums we may write $\phi$ as:
...
Since all $n_j\neq 0$ we have that the $j$th summand of $A_n$ when viewed as a multimatrix algebra includes $p^{i,j}_k$ times into the $k$th summand $A_{n+1}$. So the Bratelli diagram is indeed given by the fusion graph for $x^{?}$.
...
\begin{claim*}
A subfactor planar algebra can be embedded into the bipartite graph planar algebra of the principle graph obtained by right-acting the subfactor planar algebra's paragroup on a cyclic module category.
\end{claim*}
From the paper of Penneys and Jones we see that for any strongly markov tower of algebras, we can construct a canonical relative commutant planar algebra. It was shown that this planar algebra was isomorphic to the bipartite graph planar algebra of the Bratelli diagram of the first inclusion in the tower. We have shown that the inclusions of $\left(A_{n}\right)$ are eventually standard (the tower $\left(A_{n}\right)$ is of finite depth). Thus if we fix a number $s=2r$ such that the inclusion $A_{2r} \subset A_{2r+1} \subset A_{2r+2}$ is standard, then the canonical relative commutant planar algebra constructed on the strongly markov tower $B_{n}=A_{2r+n}$ would be isomorphic to the bipartite graph planar algebra of the principle graph of the tower $\left(A_{n}\right)$ (The principle graph of $A_{n}$ is the Bratelli diagram of the first inclusion in $\left(B_{n}\right)$). Our strategy will be to show that we can embed the space $Q_{n , +}$ into the space $A'_{2r}\cap A_{2r+n}$.\\ 

Consider the following map, $\Phi$ \todo{insert picture here} \\ 

This map sends $Q_{n , +}$ to  $A'_{2r}\cap A_{2r+n}$ and $Q_{n , -}$ to $A'_{2r+1}\cap A_{2r+n+1}$. In order to check that $\Phi$ is a planar embedding map, we use Lemma 2.48 in the paper of Penneys and Jones. 
