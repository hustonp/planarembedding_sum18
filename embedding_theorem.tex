...
Let $\cM$ be the category defined as follows.
\begin{itemize}
\item
The objects of $\cM$ are the orthogonal projections in $M_n$ for $n\geq 0$.
\item
Given projections $p\in M_n$ and $q\in M_{n+2k}$, we define
\begin{align*}
\cM(p \to q)
&:=
...
\item
For $x\in \cM(p \to q)$ and $y \in \cM(q \to r)$, we define $y \circ x:=$\nn{???}
\end{itemize}
\end{defn}
