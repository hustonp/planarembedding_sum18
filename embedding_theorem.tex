...
$$m\otimes \xalt\otimes \act=\bigoplus\limits_{k=1}^{r_{i+1}}p^{i,j}_kY^{i+1}_k$$
...
Recall that:
$$A_n=\End_{\cC}(m\otimes\xalt)\cong \End_{\cC}(\bigoplus\limits_{j=1}^{r_i}n_j Y^i_j)$$
if we have that 
...
$$A_{n+1}=\End_{\cC}(m\otimes\xalt\otimes \act)\cong \End_{\cC}(\bigoplus\limits_{j=1}^{r_i}\bigoplus\limits_{k=1}^{r_{i+1}}(p^{i,j}_kn_jY^{i+1}_k))$$
