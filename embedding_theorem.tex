...
\begin{ex}
Suppose $\cQ_\bullet$ is a finite depth subfactor planar algebra and $\cG\cP\cA(\Gamma)_\bullet$ is the graph planar algebra of the bipartite graph $\Gamma$.
Then for any $\pm$ vertex $v$ of $\Gamma$, we get a right planar module $M(\Gamma, v)_\bullet$ for $\cQ_\bullet$ by defining $M(\Gamma, v)_{n}:= p_v \cG\cP\cA(\Gamma)_{n,\pm}$, $\tr_n := p_v \tr_{\cG_{n,\pm}}$ (notice that $\tr_{\cG_{n,\pm}}$ is $\cG_{0,\pm}$-valued, so compressing by the central projection $p_v$ gives a scalar-valued trace), and $f_n : = p_v g_{n,\pm}$ where $g_{n,\pm}$ is the Jones projection for $\cG\cP\cA(\Gamma)_\bullet$.
\end{ex}
