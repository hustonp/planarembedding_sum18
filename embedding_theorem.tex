...
\nn{verification that it's a module trace!}
\end{defn}
...
Let $P_\bullet$ be a planar algebra and $M_\bullet$ a right planar module over $P_\bullet$. Let $\cC$ be the category of projections of $P_\bullet$, with $\cC_0$ the pivotal monoidal subcategory generated by the strand $X$, the full subcategory in which objects are just $\Xalt$ and $\overline{X}^{\alttens}$. We will encode the data of $M_\bullet$ in a category $\cM_0$ with an action of $\cC_0$, and then take idempotent completion to obtain the category $\cM$ of projections of $M_\bullet$, a pivotal module over $\cC$. 

We define $\cM_0$ as follows:
\begin{itemize}
	\item The objects of $\cM_0$ are $[n]$, where $n$ is a non-negative integer. The object $[0]$ should be thought of as the left wall of a tangle in the shaded planar module operad, and $[n]$ is the wall with $n$ adjacent strands. 
	\item Morphisms $[n]\to[m]$ are just members of $M_{n\to m}$, which is equal to $M_{(n+m)/2}$ as a set. Composition is defined as in $\cC_0$. For example, the composition $\cM_0([n+2k]\to[n+2k+2j])\otimes\cM_0([n]\to [n+2k])\to\cM_0([n]\to[n+2k+2j])$ is given by the action of the tangle \nn{add picture here}. 
		The composition $\cM_0([n+2k]\to[n])\otimes\cM_0([n+2k+2j]\to[n+2k])\to\cM_0([n+2k+2j]\to[n])$ is defined by the dagger of the same tangle, making it clear that $(g\circ f)^\dag=f^\dag\circ g^\dag$. 
	\item The action of $\cC_0$ on objects is given by $[n]\lhd (X^?)^{\alttens[k]}=[n+k]$, where $X^?=X$ when $n$ is even and $\overline{X}$ otherwise. The action of morphisms is given by the following tangles: 
		\nn{add}
\end{itemize}
One uses the composition of tangles in the shaded planar module operad to check that composition and the action are associative and that the action is in fact a $\dag$-bifunctor. 
\nn{
...
\begin{nota}
From this point on, we reserve the notation $(A_n,\tr_n, e_{n+1})_{n\geq 0}$ for the Jones tower of a strongly Markov inclusion of tracial von Neumann algebras and $M_\bullet = (M_n, \tr_n, e_{n+1})_{n\geq 0}$ for a Markov tower.
\end{nota}
