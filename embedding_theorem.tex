...
%Suppose $T$ is a tangle in standard form with input semidiscs of types $(k_i)_{i=1}^r$, input discs of types $((\ell_j,\pm_j))_{j=1}^s$, and output type $k_0$. 
...
%When the line at $h$ does not touch extrema of strands, $\tau_h\in\cM_{n_h}$, where $n_h$ is the sum of the number of strands and the index of the box space of the input (semi)disc (if any) at height $h$. 
...
%
%Now suppose the horizontal line passes an extremum or the top of an input (semi)disc. 
%If $\tau_h\in\cM_{n_h}$ and between $h$ and $h'$, the horizontal line passes only the top of the $i$-th input semidisc, we simply set $\tau_{h'}=\xi_i\circ\tau_h$. Of course, in general $n_i<w(h)$; then we right-include $\xi_i$ into $\cM_{w(h)}$ by $\xi_i\to\xi_i\otimes1_{x^{\alttens{w(h)-n_i}}}$. Simiarly, when passing through the top of an input disc, we include $\eta_j$ on the right and the left before compsing, where left inclusion sends $\eta_j\to 1_{mx^{\alttens{?}}}\otimes\eta_j$. Passing through the maximum of a string becomes compsing with the coevalutation $\coev_{x}$ or $\coev_{\overline{x}}$, depending on the shading, inlucded on the left and right in the same way; similarly, passing through the minimum of a string becomes composing with a suitable evaluation. 
%	
...
\begin{remark}
If $\left( M_n, \tr_n, e_{n} \right )$ is a Markov tower then we know from proposition \ref{prop:MultistepJonesProjections} that 
$\left( M_{kn}, \tr_{kn}, e_{kn} \right )$ is also a Markov tower. Let $\Gamma$ be the principal graph of  $\left( M_n, \tr_n, e_{n} \right )$ and $\Gamma '$ be the principal graph of  $\left( M_{kn}, \tr_{kn}, e_{kn} \right )$. Then
\end{remark}
...
=
d^{-2i-j-k}E_{n+i+j}
\left(yx \cdot \TLPlusPIso{n}{i}{j}\right) 
\cdot 
\TLCapJoin{n}{i}{j}{j}{i} 
\cdot 
...
\pi(y\circ x)
=
d^{-2i-j-k}E_{n+i}\left(yx\cdot \TLMinusPIso{n}{i}{j}\right)
\TLTStart\TLTThrough{n}\TLTCapcup{i}\TLTCapcup{j}\TLTCapcup{k}\TLTEnd
$$ 
First observe that using lemma \ref{lem:LeftKink} we have:
$$
d^{-i}E_{n+i}\left(E_{n+2i}\left(yx\cdot\TLMinusPIso{n}{i}{j}\right)\right)
\TLTStart\TLTThrough{n}\TLTCapcup{i}\TLTEnd
=
E_{n+2i}\left(yx\cdot\TLMinusPIso{n}{i}{j}\right)\TLTStart\TLTThrough{n}\TLTCapcup{i}\TLTEnd
$$
And since $E_{n+i}=E_{n+i}\circ E_{n+2i}$ we have that:
$$
\pi(y\circ x)
= 
d^{-i-j-k}E_{n+2i}\left(yx\cdot \TLMinusPIso{n}{i}{j}\right)\TLTStart\TLTThrough{n}\TLTCapcup{i}\TLTCapcup{j}\TLTCapcup{k}\TLTEnd
$$
Now observe that $yx\cdot\TLMinusPIso{n}{i}{j}\in M_{n+2i+j}$ and we know from proposition \ref{prop:MultistepJonesProjections} we know that $\TLTStart\TLTThrough{n}\TLTThrough{2i}\TLTCapcup{j}\TLTEnd$ implements $E^{n+2i+j}_{n+2i}$ so:
$$
...
\end{tikzpicture}
$
\item (Left Capping) In \S\ref{ssec:canonical}, % maybe cite definition. % maybe expand the explanation of the definitio, to make the claim about $\gamma_n$ clear
we discussed that the left-capping tangle in the canonical planar $\ast$-algebra is given for $n\geq 1$ by 
\[
E^{B_0'\cap B_{n}}_{B_1'\cap B_{n}}(x) = \frac{1}{d^2}\sum_{b}bxb^\ast
\]
where $\{b\}$ is a Pimnser Popa basis of $B_1$ over $B_0$. 
We will now check that left capping preserves $\Phi(Q_{\bullet})$. 
		For any $x \in Q_{n,+}$, we have 

\centerline{
  \begin{minipage}{\linewidth}
			\[
				d^2E^{B_0'\cap B_{n}}_{B_1'\cap B_{n}}(\Phi(x))
				=
\sum_{b\in B}
\begin{tikzpicture}[baseline]
...
\begin{tikzpicture}[baseline]
	\fill[ctwoshading] (-1,-2.1) -- (-1,2.1) -- (-.6,2.1) -- (-.6,-2.1) -- (-1,-2.1);
	\fill[shaded] (0,2.1) -- (0,0.7) -- (0.3,0.7) -- (0.3,0.25)arc (-180:0:.15cm)-- (0.6,1.5)arc(180:0:.3cm) -- (1.2,-1.5) arc(0:-180:0.3cm) -- (0.6,-0.25) arc(0:180:0.15cm) -- (0.3,-0.7) -- (0,-.7) -- (0,-2.1) -- (1.5,-2.1) -- (1.5,2.1);
	\draw[thick,red] (-0.6,-2.1) -- (-0.6,2.1);
	\draw (0.3,.7)--(0.3,0.25) arc (-180:0:.15cm)-- (0.6,1.5) arc(180:0:.3cm) -- (1.2,.15);
	\draw (0.3,-.7)--(0.3,-0.25) arc (180:0:.15cm)-- (0.6,-1.5) arc(-180:0:.3cm) -- (1.2,-.15);
	\draw (0,-2.1) -- (0,2.1);
