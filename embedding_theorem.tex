...
%%%%%%%%%%%%%Shifting%%%%%%%%%%%%%
Suppose that $A_0 \subset (A_1, \tr_1)$ is a strongly Markov inclusion of tracial von Neumann algebras, and let $(A_n, \tr_n, e_n)$ be its Jones tower.
We know from \cite[Prop.~2.20]{MR2812459} that the inclusion $A_0 \subseteq A_2 \subseteq \left(A_4, \tr_4,d^{-2}(e_1e_2)(e_3e_2)\right)$ is standard. 
Moreover, by \cite[Prop.~2.24]{MR2812459}, the canonical commutant conditonal expectation $E^{A'_0}_{A'_2}$ is given by
\[
 E^{A'_0}_{A'_2}(x)\quad=\quad d^{-4} \sum_{b \in B} bxb^{\ast} 
%\quad
%=
%\quad 
%d^{-2}
...
%\end{tikzpicture}
\]
where $B$ is a Pimsner-Popa basis for $A_2$ over $A_0$.
\dave{don't get to draw a picture for elements of $A_k'$, only for $A_k'\cap A_n$}
