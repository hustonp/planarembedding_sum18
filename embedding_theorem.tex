...
Because $\pi$ respects $\dag$, as in the definition above, we only need to consider 3 cass of composition:
\begin{enumerate}[label={\rm(\arabic*)}]
	\item
		\label{eq:upup}  
...
% \TLCapJoin{n}{i}{j}{j}{i} 
	\begin{tikzpicture}[baseline=.3cm]
		...
		\draw[thick, rounded corners = 5pt] (0,0) rectangle (1.6,.8);
	\end{tikzpicture}
	\intertext{But then Lemma \ref{lem:LeftKink} immediately gives that:}
	\pi(y\circ x) &= d^{-i-j-k} yx \cdot 
...
\begin{align*}
	...
	\begin{tikzpicture}[baseline=.3cm]
		...
		\draw[thick, rounded corners = 5pt] (0,0) rectangle (1.6,.8);
	\end{tikzpicture}\\
	\intertext{First observe that using lemma \ref{lem:LeftKink} we have: 
	...
	\begin{tikzpicture}[baseline=.3cm]
		...
		\draw[thick, rounded corners = 5pt] (0,0) rectangle (1.6,.8);
	\end{tikzpicture}\\
	\intertext{Now observe that $yx\cdot
	% \TLMinusPIso{n}{i}{j}
	\begin{tikzpicture}[baseline=.3cm]
		\draw (.2,0) -- (.2,.8);
		\node at (.2,1) {\scriptsize{$n$}};
		...
		\draw[thick, rounded corners = 5pt] (0,0) rectangle (1.2,.8);
	\end{tikzpicture}
	\in M_{n+2i+j}$ and we know from proposition \ref{prop:MultistepJonesProjections} we know that $
