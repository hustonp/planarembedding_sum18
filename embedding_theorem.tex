...
Let $Y^i_1,\ldots Y^i_{r_i}$, for $i=0$ or $1$ be a collection of non-isomorphic simple objects in $\cC_{2i}$ such that every object in $\cC_{2i}$ is isomorphic to a direct sum of these elements, as in the previous proof. 
	We will work the index $i$ mod 1 so that we may write $Y^{i+1}_j$ for brevity of notation. 
...
where $\overline{x}^{?}$ is $x$ if $n$ is even and $\overline{x}$ if $n$ is odd. 
...
$$A_{n+1}=\End_{\cC}(m\otimes\xalt\otimes x^{?})\cong \bigoplus\limits_{k=1}^{r_{i+1}}\End_{\cC}(\sum\limits_{j=1}^{r_i}(p^{i,j}_kn_j)Y^{i+1}_k)\cong M_{(\sum\limits_{j=1}^{r_i}p^{i,j}_k n_j)}(\bbC)$$
...
But then:
$$\phi_j \otimes \id_{x^{?}}\in \End_{\cC}(n_jY^{i}_j\otimes x^{?})\cong \End_{\cC}(\bigoplus\limits_{k=1}^{r_{i+1}}n_jp^{i,j}_kY^{i+1}_k)\cong \bigoplus\limits_{k=1}^{r_{i+1}}\End_{\cC}(n_jp^{i,j}_kY^{i+1}_k)\cong \bigoplus\limits_{k=1}^{r_{i+1}}M_{(n_jp^{i,j}_k)}(\bbC)$$
