...
Given a subfactor planar algebra $Q_{\bullet}$ and a module $\cM$ over $Q_{\bullet}$, let $\cC$ and $\widetilde{\cC}$ be the multifusion categories associated to $\cG$ and $\cM$, as described in section 2.1. Let $1_{\widetilde{\cC}}= 1_0 \oplus 1_1 \oplus 1_2$ be the tensor unit in $\widetilde{\cC}$, indexed so that $\cC$ is the non-unital $2\times 2$ unitary multifusion subcategory with $1_\cC = 1_0\oplus 1_1$, and $\cM\cong\cC_{20}\oplus \cC_{21}$. Overall,
$$
\cM 
=
\begin{pmatrix}
...
\begin{proof}
 ...
 For some nonnegative integers $n_1,\ldots n_r$ so then we have:
	...
	If there are $s$ ismorphism classes of simple objects in $\cC_{21}$ we obtain a similar result. So for any $n$ the Bratteli diagram for $A_n\subseteq A_{n+1}$ has at most $r+s$ vertices, so the principal graph must be finite. 
\end{proof}
