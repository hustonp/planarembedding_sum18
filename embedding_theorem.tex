...
, and well-definedness of certain maps. We will need the following:
...
\begin{defn} The canonical planar $\ast$-algebra of a strongly Markov inclusion $M_0\subseteq (M_1,\tr_1)$ is defined as follows:

	The box spaces are $P_{n,+}:=\theta_n^{-1}(M_0'\cap M_{n}) $ and $P_{n,-}:=\theta_n^{-1}(M_1'\cap M_{n+1})$. For a tangle, $T$, in standard form, the action of $T$ can be understood by reading the tangle from top to bottom, moving a horizontal line upwards. Suppose $T$ has type $(r,\pm)$ and $s$ input rectangles of types $(r_i,\pm_i)$, and choose a pure tensor $\nu=\otimes_{i=1}^r\nu_i\in\otimes_{i=1}^rP_{r_i,\pm_i}$. Let $n_y$ be the number of shaded regions intersected by a horizontal line at a height $y$. We first consider $y$ such that the horizontal line intersects no input disks or extrema of strands. At this level, one can read off an $M_i$-invariant element $\nu_y\in \otimes^{n_y}M_1$, where $i$ is 0 if $T$ has type $(r,+)$ and 1 if $T$ has type $(r,-)$. One should think of shaded regions as representing elements of $M_1$ and the unshaded regions as representing $\otimes_{M_0}$. At the bottom of a tangle, $n_y=0$ or $1$, and we say $\nu_y=1\in M_i$. We will now describe what happens when the line moves upwards and passes through extrema of the strings and input rectangles:
	\begin{itemize}
