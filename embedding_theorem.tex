...
Thus, at each level of the Bratteli diagram corresponding to $A_n$ % should be an inclusion, 
	there are at most $\max\{r_1,r_0\}$ vertices, so the principal graph must be finite. % Of course, this is the principal graph of this particular Markov tower, perhaps not the principal graph of some ancient II_1 factor. The fact(?) that Popa's construction preserves the principal graph is important. 
\end{proof}
