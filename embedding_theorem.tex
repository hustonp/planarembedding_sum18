...
By definition, we have assumed $\Tr_k$ to be tracial and positive on endomorphisms. The trace is also compatible with the action of $\C$, which can be seen by composing the trace tangle with the action tangles. % draw?
To see that the $\Tr_k$ give a positive module trace, we reduce the general case to the case of endomorphisms as follows: suppose $f\in\cM([n]\to[n+2k])$. Obtain $g\in\C(1\to(X^?)^{\alttens[2k]})$ from $1\in\cP_{k,\pm}$. Then \[\Tr_n(f^\dag\circ f)=\Tr_{n+2k}((f\vartriangleleft g)^\dag\circ(f\vartriangleleft g))\Tr_{2k}(g^\dag\circ g)^{-1}=d^k\Tr_{n+2k}((f\vartriangleleft g)^\dag\circ(f\vartriangleleft g))\]
\nn{make a picture instead of this, probably}
Therefore, it suffices to show that if $f\neq0$, then $f\vartriangleleft g\neq0$. We know that $g\neq 0$, since $g$ is the image of the identity of a non-zero algebra under the action of an invertible tangle. Since $\cC$ has all duals, by Frobenius reciprocity, the action of a non-zero morphism of the appropriate shading is faithful, completing the proof. 
\end{defn}
...
Recall that we need only verify $\pi$ respects three types of composition:
	...
	\end{enumerate}
\eqref{upup} Let $x:[n]\rightarrow [n+2i]$ and $y:[n+2i]\rightarrow [n+2i+2j]$. Then $\pi(y\circ x)=E_{n+i+j}(yx{}^nh^i_j){}^n r^{i+j}F^{n+2i+2j+k}_{n+2i+2j}$. Applying Lemma \ref{lem:LeftKink} and noting that: 
...
\eqref{UPdown} Let $x:[n]\rightarrow [n+2i+2j]$ and $y:[n+2i+2j]\rightarrow [n+2i]$. Then $\pi(y\circ x)=E_{n+i}(yx){}^ng^i_j)F^{n+i}_{n}F^{n+2i+j}_{n+2i}F^{n+2i+2j+k}_{n+2i+2j}$. 
