...
\begin{prop} A Markov sequence satisfies the following elementary properties for $n\geq 1$.
\begin{enumerate}[(A)]
\item
\label{EP:Injective}
...
Since $X_0 = (0)$ and $X_1 = (0)$ by definition, we may assume $n\geq 2$.
		As in the proof of \cite[Thm.~4.6.3.vi]{MR999799}, % removed: ``and (5)''. Mysterious.
		we may assume $y$ is a central projection in $M_{n+1}$ such that $y e_{n} = 0$.
Then for all $ae_{n-1} b \in X_n$, we have $y ae_n b = d^2 yae_n e_{n+1}e_n b = d^2 ae_n ye_{n+1} e_n b = 0$.
The final claim follows because $z_{n}E_{n+1}(y) = E_{n+1}(z_n y)= 0$, where $z_n$ is the central support of $e_{n-1}$ in $M_n$.
...
We have checked that $\Phi(Q_{\bullet}) \subset P_{\bullet}$ is a $\ast$-planar algebra inclusion. Let $G_\bullet$ be the bipartite graph planar algebra on the principle graph of $(A_n)$. Then by Theorem 3.33 in \cite{penneys}
