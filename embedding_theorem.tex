...
\item
unitary $2\times 2$ multitensor categories with orthogonal decomposition into simples $1_\cC = 1_0 \oplus 1_1$ and generator $X = 1_0 \otimes X \otimes 1_1$
(see also \cite{1808.00323}),\footnote{
\nn{define generator}
} 
\item
Jones' subfactor planar algebras \cite{math.QA/9909027}, and
\item
Popa's standard $\lambda$-lattices \cite{MR1334479}.
\end{enumerate}
...
A \emph{right planar module} for a subfactor planar algebra $\cP_\bullet$ is a tower of finite dimensional $\dag$-algebras $(M_k)_{k\geq 0}$ with $\dim(M_0) = \bbC$ together with an action of the 
\dave{marble Swiss cheese operad?}
\emph{shaded Swiss cheese tangle operad}, 
which is a variation of Voronov's Swiss cheese operad \cite{MR1718089}.
We refer the reader to \S\nn{} for the precise definition, but we include a representative tangle below which acts amongst the algebras $M_k$ and the box spaces $\cP_{n,\pm}$:
$$
...
%
	\halfcircle{}{(0,0)}{2}{}
%
	\openhalfcircle{}{(0,1)}{.3}{0}{$b$}
	\openhalfcircle{}{(0,-1)}{.3}{.4}{$a$}
	\ncircle{unshaded}{(2.5,1)}{.4}{180}{$3$}
	\ncircle{unshaded}{(1.5,0)}{.4}{0}{$2$}
	\ncircle{unshaded}{(2.5,-1)}{.4}{90}{$1$}
\end{tikzpicture}
\,:
(M_3 \otimes M_1) \otimes (\cP_{2,+}\otimes \cP_{1,-} \otimes \cP_{3,+}) \to M_{4}
$$
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Modules for unitary multitensor categories, subfactor planar algebras, and \texorpdfstring{$\lambda$}{lambda}-lattices} 
...
\dave{here's a labelled representative tangle (without strings or shadings) from the operad i want to use.
Perhaps this is in \cite{MR1718089}? The name is right...}
$$
\begin{tikzpicture}[baseline = -.1cm]
	\filldraw[shaded] (.3,1.3) .. controls ++(90:.3cm) and ++(270:.3cm) .. (.8,2) -- 
		(1.8,2) .. controls ++(270:.3cm) and ++(90:.3cm) .. (2.5,1.4) --
...
\end{tikzpicture}
\,:
(M_3 \otimes M_1) \otimes (Q_{2,+}\otimes Q_{1,-} \otimes Q_{3,+}) \to M_{4}
$$
%\end{defn}
%\begin{defn}
...
\begin{ex}
Given a subfactor planar algebra $\cQ_\bullet$, the even parts $\cQ_\pm :=( \cQ_{n,\pm}, \tr_{n,\pm}, e_{n,\pm})$ are each separately cyclic right planar modules for $\cQ_\bullet$.
\end{ex}
