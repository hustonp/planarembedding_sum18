...
\dave{do we always use $A_0 \subset (A_1, \tr_1)$ for a strongly Markov inclusion and $(M_n, \tr_n, e_n)$ for a Markov sequence?
...
Notice that for all $pxp \in pA_1 p$, we have $e_1p (pxp) e_1p = p e_1xe_1p = pE_1(x)e_1p = E_1(pxp)e_1p$, so the conditional expectation is implemented by $e_1p$.
...
\begin{lem}
Under the additional assumption that $A_0pA_0 = A_0$,\footnote{The condition that $A_0pA_0$ implies that the central support of $p$ in $A_0$ is $1$.}
...
Then $pA_0p \subset (pA_1p, \tr_1^p)$ is strongly Markov.
\end{prop}
\begin{proof}
\nn{TODO}
\end{proof}
...
\begin{cor}
Assume the hypotheses of Lemma \ref{prop:CompressingStronglyMarkovInclusion}, and let $(A_n, \tr_n, e_n)$ be the Jones tower for $A_0 \subset (A_1, \tr_1)$.
...
\dave{false; only true if $(A_n)$ is the Jones tower of a strongly markov inclusion.}
