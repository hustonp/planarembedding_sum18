...
(see \cite{MR2091457,MR4133163}),\footnote{
...
$$
where $(\overline{c}, \ev_c, \coev_c)$ is the canonical \emph{balanced} dual of $c\in \cC$ \cite{MR1444286,MR2091457,MR3342166,MR4133163}.
...
One passes from (1) to (2) using the diagrammatic calculus for module categories, similar to how one gets a subfactor planar algebra from $(\cC, X)$ via the diagrammatic calculus for pivotal categories \cite{MR2811311,MR4133163}.
...
Now any pointed bipartite graph $(\Gamma,v_0)$ with a \emph{quantum dimension function} on vertices $\dim: V(\Gamma) \to \bbR_{>0}$ satisfying
$$
...
pointed connected locally finite bipartite graphs $(\Gamma, v_0)$ with a quantum dimension function.
\end{coralpha}

It would be interesting to determine
under the Tannaka-Krein duality of \cite{MR3420332}
which property on the (Morita class) of quantum homogeneous space over $SU_q(2)$
corresponds to the existence of a compatible module trace $\Tr^\cM$ on the corresponding module category $\cM$.
...
In this section, we rapidly recall the definitions of a subfactor planar algebra \cite{math.QA/9909027} and its unitary $2\times 2$ multitensor category of projections \cite{MR2811311,MR4133163}.
...
Here, $(\overline{X}, \ev_X, \coev_X)$ is the canonical \emph{balanced} dual of $X$ \cite{MR3342166,MR3994584,MR4133163} which satisfies the zig-zag axioms and the balancing equation
$$
...
We refer the reader to \cite{1607.06041,MR4133163} for more details.}
\[
\left\{\, 
...
Starting with a subfactor planar algebra $\cP_\bullet$, one may form its unitary $2\times 2$ multitensor category of projections $\cC$ \cite{MR2559686,MR3405915,MR4133163}, which comes with a canonical generator corresponding to the unshaded-shaded strand in $\cP_{1,+}$, and the canonical spherical unitary dual functor \cite{MR3342166,MR3994584,MR4133163}.
...
We refer the reader to \cite{MR2811311,MR4133163} for more details.
...
and $\Phi: \cP_\bullet \to \cG_\bullet$ is a planar algebra embedding.
...
\begin{ex}
Suppose $\mathcal{P}_\bullet$ is a subfactor planar algebra of modulus $d$.
...
\end{tikzpicture}
\]
Consequently, the conditional expectation $E_{n+1}:\mathcal{P}_{n+1,+}\to\mathcal{P}_{n,+}$ is given by
\[E_{n+1}(x):=\frac{1}{d}\,
\begin{tikzpicture}[baseline = -.1cm]
\draw (0,-1.2) -- (0,1.2);
...
$\frac{1}{d}\cdot\begin{tikzpicture}[baseline = .3cm]
...
The algebra $TLJ(d)_n$ is the $(n,+)$ box space of the Temperley-Lieb subfactor planar algebra $\cT\cL\cJ(d)_\bullet$ of \cite{math.QA/9909027} with loop parameter $d$.
...
Suppose $(\cM,m, \Tr^\cM)$ is a cyclic pivotal $\cT\cL\cJ(d)_\bullet$-module $\Cstar$ category.
Let $X\in \cT\cL\cJ(d)_{1,+}$ be the generating object corresponding to the unshaded-shaded strand.
...
\begin{ex}
Suppose $(\Gamma, v_0)$ is a locally finite pointed connected bipartite graph with countably many vertices, and $\dim : V(\Gamma) \to \bbR_{>0}$ is a quantum dimension function satisfying \eqref{eq:QuantumDimension}.
We construct a connected Markov tower $M_\bullet$ by defining $M_0 = \bbC$, and inductively constructing each $M_k$ as dictated by the principal graph $\Gamma$ starting at $v_0$ in the usual way \cite{MR999799,MR1473221}.
...
It is straightforward to check that $M_\bullet$ has principal graph $\Gamma$ with basepoint $v_0$ corresponding to $1_{M_0}$.
...
Connected Markov towers $M_\bullet$ are classified up to $*$-isomorphism by pointed bipartite graphs $(\Gamma,v_0)$ with a quantum dimension function $\dim: V(\Gamma) \to \bbR_{>0}$ satisfying \eqref{eq:QuantumDimension}.
\end{prop}
%\begin{proof}
...
$$
Here, 
$C$ is a Pimsner-Popa basis for $A_1$ over $A_0$
and
$B:= \set{c_k \otimes \cdots \otimes c_1}{c_1,\dots,c_k\in C}$.
It is straightforward to see that $\theta_k^{-1}(B)$ is a Pimsner-Popa basis for $A_k$ over $A_0$.
...
\draw (-.15,.3) arc(0:180:.15) -- (-.45,-.3) arc(-180:0:.15);
\draw (-.75,-.7) -- (-.75,.7);
\draw (0,-.7) node[below]{$\scriptstyle n$} -- (0,.7);
\roundNbox{unshaded}{(0,0)}{.3}{0}{0}{$x$};
\node at (-.55,0) {\tiny{$k$}};
\node at (-.85,0) {\tiny{$k$}};
%\node at (0,0) {\scriptsize{$x$}};
...
x_0mnp\xi.
$$
Thus $x_0$ commutes with $M'$ on $K$, and so $x_0=x_0q$ commutes with $M'$ on $H$, showing that $x_0\in M$, as claimed.
...
\item[\underline{Proof of (2):}]
Given that there exists a Pimsner-Popa basis for $pA_1p$ over $pA_0p$ by part (1), the inclusion is strongly Markov if and only if the Watatani index \cite{MR996807} is a scalar by \cite[1.1.4(c)]{MR1278111}.
We now calculate
$$
\sum_{pbap} pbap(pbap)^*
=
...
pA_1'\cap pA_{n+1}p.
$$
Since the central support of $p$ is $1$, by Lemma \ref{lem:CompressRelativeCommutant}, the map $\Phi_{n,\pm}:x\mapsto xp$ gives an isomorphism of von Neumann algebras $\Phi_{n,\pm}:\cP_{n,\pm}\to \cQ_{n,\pm}$ for each $n\geq 0$.
...
First, by Proposition \ref{prop:LeftCapping}, if $\{b\}$ is any Pimsner-Popa basis for $A_1$ over $A_0$, then for all $x\in \cP_{n,+}= A_0'\cap A_n$,
$$
...
We take the Pimsner-Popa basis $\{a\}$ for $pA_1p$ over $pA_0p$ and take the disjoint union with $\{(1-p)c\}$, where $\{c\}$ is a Pimsner-Popa basis for $A_1$ over $A_0$.
We now claim $\{b\} = \{a\}\cup \{(1-p)c\}$ is also a Pimsner-Popa basis for $A_1$ over $A_0$.
Indeed, since $a = pap \in pA_1p$ for all $a\in \{a\}$, we have
$$
\sum_{b} b e_1 b^*
=
\sum_{a} ape_1 a^* + \sum_{c} (1-p)ce_1 c^*(1-p)
%=
...
p\sum_{b} b xb^*
= 
p\left(\sum_{a} a(px)a^* + \sum_{b} (1-p)cx c^*(1-p) \right)
=
\sum_{a} apxa^*.
$$
