...
\newcommand{\TLPlusPIso}[3]{
	\begin{tikzpicture}[baseline = .3cm]
		...
		\node at (.2,1) {\scriptsize{$#1$}};
		\node at (.5,1) {\scriptsize{$#2$}};
		\node at (1.2,1) {\scriptsize{$#3$}};
		\node at (.8,-.2) {\scriptsize{$#2$}};
	\end{tikzpicture}
}
...
\newcommand{\TLMinusPIso}[3]{
	\begin{tikzpicture}[baseline = .3cm]
		...
		\node at (.5,1) {\scriptsize{$#3$}};
		\node at (.5,-.2) {\scriptsize{$#2$}};
	\end{tikzpicture}
}
...
\newcommand{\TLCapJoin}[5]{
	\begin{tikzpicture}[baseline=.3cm]
		...
		\node at (.2,1) {\scriptsize{$#1$}};
		\node at (.4,1) {\scriptsize{$#5$}};
		\node at (.6,1) {\scriptsize{$#4$}};
		\node at (.4,-.2) {\scriptsize{$#2$}};
...
\newcommand{\TLCapSplit}[5]{
	\begin{tikzpicture}[baseline=.3cm]
		\draw[thick, rounded corners = 5pt] (0,0) rectangle (1.6,.8);
		\draw (.2,0) -- (.2,.8);
		\draw (0.4,0) arc (180:0:.5);
		\draw (0.6,0) arc (180:0:.3);
		\draw (0.4,.8) arc (-180:0:.2);
		\draw (1,.8) arc (-180:0:.2);
		\node at (.2,1) {\scriptsize{$#1$}};
		\node at (.4,-.2) {\scriptsize{$#5$}};
		\node at (.6,-.2) {\scriptsize{$#4$}};
		\node at (.4,1) {\scriptsize{$#2$}};
		\node at (1,1) {\scriptsize{$#3$}};
	\end{tikzpicture}
}

% a set of commands to support easily describing projections
% example: 
% \[\TLProjStart\TLProjThrough{n}\TLProjCapcup{i}\TLProjThrough{k}\TLProjCapcup{\ell}\TLProjEnd\]
% If anyone wants, they can probably generalize this more slickly into a system to support dashed vertical concatenation, etc., and/or recast the above 4 commands in similar language. 
\newcommand{\TLProjEnd}{
	\draw[thick, rounded corners = 5pt] (0,0) rectangle ($ (TLPlead) + (0,.8) $);
 \end{tikzpicture}
}

\newcommand{\TLProjStart}{
 \begin{tikzpicture}[baseline=.3cm]
	\coordinate (TLPlead) at (.2,0);
}
 
% #1 - label
\newcommand{\TLProjThrough}[1]{
	\draw (TLPlead) -- ($ (TLPlead) + (0,.8) $);
	\node at ($ (TLPlead) + (0,1) $) {\scriptsize{$#1$}};
	\coordinate (TLPlead) at ($ (TLPlead) + (.2,0) $);
}

% #1 - label
\newcommand{\TLProjCapcup}[1]{
	\draw (TLPlead) arc (180:0:.2);
	\draw ($ (TLPlead) + (0,.8) $) arc (-180:0:.2);
	\node at ($ (TLPlead) + (0,1) $) {\scriptsize{$#1$}};
	\coordinate (TLPlead) at ($ (TLPlead) + (.6,0) $);
}
