...
The proof in \cite{EH3} is mostly in the language of tensor and module categories.
...
Suppose $M_\bullet=(M_n, \tr_, e_{n+1})$ is a Markov tower, and let $j\geq 0$ and $k\geq 1$.
...
\nn{There must be a mistake here, since $j=k$ should be the same for both.
I think there is only one definition, the first one, and the second is just wrong.}
...
For $f\in \cM([n]\to[m])$ where $m<n$, we define $f\vartriangleleft 1 := (f^\dag \vartriangleleft 1)^\dag$.
 ...
 We prove the case $f:[n]_{\cM}\to[n+2k]_{\cM}$ and $g:[m]_{\TLJ}\to [m+2\ell]_{\TLJ}$, where both $m\ge k$ and $\ell\ge k$. 
...
(1\vartriangleleft g)\circ(f\vartriangleleft 1)
 ...
 We describe a representative case. 
 Suppose $f:[n]_{\cM}\to[n+2i+2j]_{\cM}$ and $g:[n+2i+2j]_{\cM}\to[n+2i]_{\cM}$, and suppose $m=k+i+j$. Then
 \begin{align*}
