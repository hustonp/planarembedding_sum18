...
\nn{\begin{itemize}
\item
Examples constructed via graph planar algebra embedding
\cite{MR1929335,MR2511128,MR2679382,MR2979509,MR2822034,MR3314808,MR3394622,MR3402358,MR3306607,EH3}. 
\item
Can always do by GPA embedding theorem.
\item
A couple are not into the principal graph
\item
Can also embed into fusion graph for a module
\item
if and only if from \cite{EH3}.
\item
old school towers of algebras approach \cite{MR936086,MR999799,MR1278111}
\end{itemize}}


\nn{TODO: more intro stuff here}
...
By a \emph{pivotal module category} for $\cQ_\bullet$, we mean a finitely semisimple $\Cstar$ category $\cM$ which is an indecomposable right $\cC$-module category, together with a faithful positive trace $\Tr^\cM_m$ on each endomorphism $\Cstar$ algebra $\End_\cM(m)$ which is compatible with the left $\cC$-action (see \cite{MR3019263,EH3}).
That is, for all $f\in \End_\cM(m \vartriangleleft c)$, 
$$
...
%Here, we use the graphical notation of \nn{}, where coupons in $\cM$ have no boundary on the left to indicate the absence of a left $\cC$-action.

From the right pivotal $\cC$-module $\Cstar$ category $(\cM, \Tr^\cM)$ together with a choice of simple object $m\in \cM$, we build a tower of finite dimensional von Neumann algebras 
$$
...
Since $\cM$ is finitely semisimple, the Markov tower $(M_n, \tr_n, e_{n+1})$ has \emph{finite depth}, meaning the principal graph is finite.
This principal graph is exactly the \emph{fusion graph} of $(\cM,m)$ with respect to $c\in \cC$.
...
Moreover, by \cite[Thm.~3.8]{MR2812459}, the planar algebra $\cP_\bullet$ is \emph{non-canonically} isomorphic to the bipartite graph planar algebra $\cG_\bullet$ of the Bratteli diagram $\Gamma$ of the inclusion $A_0 \subset A_1$. 
...
Notice that we may canonically identify $\Gamma$ with the principal graph of the Markov tower $(M_n, \tr_n, e_n)$ and the fusion graph for $(\cM,m)$ with respect to $c\in \cC$.

We are now in the position to state the embedding theorem for module categories for a finite depth subfactor planar algebra.

\begin{thmalpha}
The unital $*$-algebra maps $\Phi_{n,\pm}:= \id_m \vartriangleleft \id_{(c\otimes \overline{c})^{\otimes r}}\vartriangleleft - : \cQ_{n,\pm} \to \cP_{n,\pm}$
$$
\begin{tikzpicture}[baseline]
	\draw (0,-.7) -- (0,.7);
...
Similarly, we get an embedding into the graph planar algebra of the dual principal graph from choosing $\cM= \cC_{10} \oplus \cC_{11}$ and $m = 1_{\cC_{11}}$ corresponding to the shaded empty diagram.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{The canonical planar algebra of a strongly Markov inclusion} 
...
From any subfactor planar algebra, one may construct a 2-category. We refer the reader to \cite{paragroup} and \cite{paragroup2} for more details. We adopt the definitions used in \cite{paragroup}, since we wish to work with shaded planar algebras.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Modules for planar algebras and pivotal module categories}
...
\item
This follows directly from \ref{EP:Injective}.
...
\item
$M_ne_nM_n$ is a 2-sided ideal of $M_{n+1}$ by the pull down condition, as discussed in Remark \ref{pulldowniff}.
...
Hence $$0 = \sum E_{n}(aa_i)E_n(b_ib)e_n = e_na\left(\sum a_i e_n b_i\right) be_n$$ for all $a,b\in M_n$, and thus $\sum a_i e_n b_i = 0$, so the map is injective. 
...
Because of this and the fact that the Bratteli diagram for $M_0\subseteq M_1$ is part of the principal graph we can ``unravel" the Bratteli diagram for $M_{n}\subseteq M_{n+1}$ to obtain the principal graph for the Markov tower $(A_n,\tr_n)$.
...
\begin{ex}
The Temperley-Lieb algebras of modulus $d\geq 2$ with the usual Jones projections form a Markov tower. Their principal graph is $A_{\infty}$.
\end{ex}
...
\begin{ex}[Shifting a Markov tower]
Suppose $(M_n, \tr_n, e_{n+1})$ is a Markov tower.
For any $k\geq 1$, $(M_{n+k}, \tr_{n+k}, e_{n+k+1})$ is also a Markov tower.
\end{ex}

Similar to the discussion in \S\ref{sec:CompressionIso}, given a Markov tower $(M_n,\tr_n, e_{n+1})$, we obtain another Markov tower by compression by a non-zero projection $p\in P(M_0)$.
...
\end{prop}
\begin{proof}
First, it is easy to see that the projections $(pe_n)_{n\geq 1}$ satisfy the Temperley-Lieb-Jones relations, since $[e_n,p]=0$ for all $n\geq 0$.
That $pe_n$ implements the trace-preserving conditional expectation $pM_np \to pM_{n-1}p$ was shown above in \eqref{eq:CompressionImplementsConditionalExpectation}.
Using \eqref{eq:CompressedConditionalExpectation}, this immediately implies that $E_{n+1}^p(pe_n) = pE_{n+1}(e_n) = d^{-2}p = d^{-2} 1_{M_n}$.
Finally, for all $n\geq 1$, $pM_{n+1}p (pe_n) = pM_{n+1}e_np = pM_ne_np = pM_npe_n$, so we have pull down.
\end{proof}
...
Ordinarily, for subfactors and planar algebras, Kauffman diagrams are drawn with strings going from \emph{bottom to top}. We put the number $k$ above or next to a strand to denote a bundle of $k$ parallel strands, and the label is omitted for single strands.
...
f^{n+k}_n
&:=
d^{k(k-1)}(e_{n+k}e_{n+k-1}\cdots e_{n+1})(e_{n+k+1}e_{n+k}\cdots e_{n+2})\cdots(e_{n+2k-1}e_{n+2k-2}\cdots e_{n+k})
\\
F^{n+k}_n
&:=
\begin{tikzpicture}[baseline = .3cm]
...
The following lemma follows from simple induction arguments.

\begin{lem}
\label{lem:MultistepJonesProjections}
Fix $n\geq 0$ and $k\geq 1$.
For $i\in \bbN$, define the $k$-cabled Jones projections
$
g_i := f^{n+ik}_{n+(i-1)k}
$.
The projections $(g_i)_{i\in \bbN}$ satisfy the Temperley-Lieb-Jones relations \ref{eq:MarkovJonesProjections} with $d^{-2}$ replaced with $d^{-2k}$.
Moreover, if $M_\bullet$ is a Markov tower, $g_i \in M_{n+(i+1)k}$, and the $(g_i)_{i\in \bbN}$ satisfy properties
\ref{eq:MarkovImplement} and \ref{eq:MarkovIndex} with respect to the multi-step tower $(M_{n+ik}, \tr_{n+ik}, e_{n+(i+1)k})_{i\geq 0}$, i.e.,
\begin{itemize}
\item
For all $x\in M_{n+ik}$, $g_i x g_i = E^{n+ik}_{n+(i-1)k}(x) g_i$, where $E^{n+ik}_{n+(i-1)k}: M_{n+ik} \to M_{n+(i-1)k}$ is the canonical faithful trace-preserving conditional expectation.
\item
$E^{n+(i+1)k}_{n+ik}(g_i) = d^{-2k}$.
\end{itemize}
\nn{Does \ref{eq:MarkovPullDown} hold?}
\end{lem}
...
A better way to prove associativity, which also simultaneously proves $\cM_0$ is $\Cstar$,\footnote{
...
is to prove that each $4\times 4$ (possibly non-associative) \emph{linking algebra} \cite{MR808930} 
$\cL:=(\cM_0([a] \to[b]))_{a,b}$ where $a,b\in \{n, n+2j, n+2j+2k, n+2j+2k+2\ell\}$
is $\dag/*$-isomorphic to a (associative!) von Neumann algebra.
...
\right).
$$
Using Lemma \ref{lem:MultistepJonesProjections}, one checks the map $\pi$ is an injective unital algebra map satisfying $\pi(x^\dag) = \pi(x)^*$, and is thus an isomorphism onto its image.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Modules for pivotal categories, planar algebras, and \texorpdfstring{$\lambda$}{lambda}-lattices} 
...
\item
a $\lambda$-lattice $(A_{n,\pm})$.\footnote{
\nn{fix:
We will actually have a $\lambda$-sequence, which is equivalent to a $\lambda$-lattice.}
}
\end{itemize}

\nn{fix below here.}

Suppose $\cQ_\bullet$ is a subfactor planar algebra.
We denote the Jones projections of $\cQ_\bullet$ by $e_{n,\pm}$. 

\begin{defn}
A right \emph{planar module} for $\cQ_\bullet$ is a Markov tower $M_\bullet = (M_n, \tr_n, f_n)$ together with a family of unital $*$-algebra homomorphisms $\rho_{\bullet, \bullet}: M_\bullet \otimes \cP_\bullet \to \cP_\bullet$ where for $k\geq 0$,
$$
\rho_{2k, n} : M_{2k} \otimes \cP_{n,+} \to M_{2k+n}
\qquad
\qquad
\rho_{2k+1, n} : M_{2k+1} \otimes \cP_{n,-} \to M_{2k+n+1}
$$ 
satisfying the following axioms: 
\begin{enumerate}[label={\rm(\arabic*)}]
\item
For all $n\geq 1$, $\rho_{k,n+1}(e_{n}) = f_{n+k}$, where $e_n = e_{n,+}$ if $k$ is even and $e_n = e_{n,-}$ if $k$ is odd.
(Notice this implies that the Jones projections $(e_{n,\pm})$ and $(f_n)$ have the same loop modulus $d$.)
\item
...
\end{tikzpicture}
\,\,
\right) = d\cdot E^{M_{k+n}}_{M_{k+n-1}}(\rho_{k,n}(m\otimes x))$.
\end{enumerate}
%\item
%For all $x\in \cP_{n,\pm}$ and $m\in M_k$, we have
...
\begin{ex}
Any Markov tower $M_\bullet = (M_n, \tr_n, f_n)$ with loop modulus $d$ is a cyclic right planar module for $\cT\cL\cJ(d)_\bullet$ where $\rho : M_\bullet \otimes \cT\cL\cJ(d)_\bullet \to M_\bullet$ is given by $\rho_{k,n+1}(e_n) := f_n$.
\end{ex}

\begin{ex}
Given a subfactor planar algebra $\cQ_\bullet$, both $\pm$ parts of $\cQ_\bullet$ given by $\cQ_\pm :=( \cQ_{n,\pm}, \tr_{n,\pm}, e_{n,\pm})$ are cyclic right planar modules for $\cQ_\bullet$.
\end{ex}
...
Then for any $\pm$ vertex $v$ of $\Gamma$, we get a Markov tower $M_\bullet = M(\Gamma,v)_\bullet$ by defining $M(\Gamma, v)_{n}:= p_v \cG_{n,\pm}$, $\tr_n := p_v \tr_{\cG_{n,\pm}}$ (notice that $\tr_{\cG_{n,\pm}}$ is $\cG_{0,\pm}$-valued, so compressing by the central projection $p_v$ gives a scalar-valued trace), and $f_n : = p_v g_{n,\pm}$.

Now if $\cQ_\bullet$ is a finite depth subfactor planar algebra and $\Phi : \cQ_\bullet \hookrightarrow \cG_\bullet$ is a planar algebra embedding, then we may endow $M_\bullet$ with the structure of a cyclic right planar $\cQ_\bullet$ module by defining 
$$
...
\begin{lem}
Suppose $\cQ_\bullet$ is a finite depth subfactor planar algebra and $M_\bullet$ is a right planar module for $\cQ_\bullet$.
Then $M_\bullet$ has finite depth.
\end{lem}
\begin{proof}
Let $r$ be minimal such that $\cQ_{r+1,+} = \cQ_{r,+} e_{r,+}\cQ_{r,+}$, and let $\{b\}$ be a Pimsner-Popa basis for $\cQ_{r+1,+}$ over $\cQ_{r,+}$ so that $\sum_b b (e_{r,+}) b^* = 1_{\cQ_{r+1,+}}$.
Then 
\begin{align*}
1_{M_{r+1}}
&=
\rho_{0,r+1}(1_{M_0} \otimes 1_{\cQ_{r+1,+}})
=
\sum_b
\rho_{0,r+1}(1_{M_0}\otimes b(e_{r,+})b^*)
\\&=
\sum_b
\rho_{0,r+1}(1_{M_0}\otimes b)
\rho_{0,r+1}(1_{M_0}\otimes e_{r,+})
\rho_{0,r+1}(1_{M_0}\otimes b^*)
%d^{-1}
%\sum_b
%\rho_{0,r+1}\left(
...
\\&=
\sum_b
\rho_{0,r}(1_{M_0}\otimes b)
(f_r)
\rho_{0,r}(1_{M_0}\otimes b^*).
\end{align*}
Hence $\{\rho_{0,r}(1_{M_0} \otimes b)\}$ is a Pimsner-Popa basis for $M_{r+1}$ over $M_r$.
\end{proof}

\begin{defn}
Let $\cC$ be a unitary $2\times 2$ multitensor category. % generated by $X \in \cC_{01}$.
...
When $\cC$ is generated by a single $X\in \cC_{01}$ and $(\cM,\Tr^\cM,m)$ is a pointed pivotal right module category, we define the \emph{cyclic pivotal right module category} $\cM_{m,X}$ to be the (non Karoubi complete!) full subcategory of $\cM$ whose objects are of the form $m \otimes X^{ \text{alt}\otimes k}$ for $k\geq 0$, which is a pointed pivotal right module category over $\cC_X$, the (non Karoubi complete!) full subcategory of $\cC$ whose objects are of the form $ X^{ \text{alt}\otimes k}$ for $k\geq 0$.
\end{defn}
...
Let $\cQ_\bullet$ be a subfactor planar algebra,
let $\cC$ be its $2\times 2$ unitary multitensor category of projections,
and let $X \in \cQ_{1,+}$ be the unshaded-shaded strand (so that we may also view $X\in \cC_{01}$).
There is a bijective correspondence between equivalence classes
\[
\left\{\,
\parbox{7.7cm}{\rm 
cyclic pivotal right module $\Cstar$ categories for $\cC$}
\,\right\}
\,\,\cong\,\,
\left\{\,\parbox{5.7cm}{\rm 
cyclic right planar modules for $\cQ_\bullet$
}\,\right\}.
\]
\end{thm}
...
\nn{When $M_\bullet$ is a cyclic right planar module for $\cQ_\bullet$ with projection categories $\cM$ and $\cC$ respectively, endow $\cM$ with the structure of a cyclic right $\cC$-module $\Cstar$ category.}
