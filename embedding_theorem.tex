...
$$
\nn{\text{tangle composition example}}
$$
...
\begin{rem}
Given a subfactor planar algebra $\cP_\bullet$, we get two Markov towers $\cP_\pm= (\cP_{n,\pm})_{n\geq 0}$ with traces
$$
\tr_{n,\pm}:= d^{-n}
\nn{todo}
$$
and Jones projections
$$
e_{n,+}:=
\nn{todo}
\qquad
...
From a subfactor planar algebra, one may form its unitary $2\times 2$ multitensor category of projections $\cC$ \cite{MR2811311,MR2559686,MR3405915}, which comes with a canonical generator corresponding to the unshaded-shaded strand in $\cP_{1,+}$, and the canonical spherical unitary dual functor \cite{1808.00323}

\begin{defn}
A unitary $2\times 2$ multitensor category is an indecomposable rigid $\Cstar$ tensor category which is Karoubi complete such that $1_\cC$ has an orthogonal decomposition into simple objects as $1_\cC = 1_0 \oplus 1_1$.
...
From any subfactor planar algebra, one may construct a 2-category. We refer the reader to \cite{paragroup} and \cite{MR2559686} for more details. 
