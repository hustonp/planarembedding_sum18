...
In this case, we can canonically identify $\cM_0 = \bbC$ as a $\Cstar$-algebra, and each $\Tr_n$ is a scalar-valued.
We define $\tr_n := d^{-n} \Tr_n$, where $d$ is the loop parameter of $\cP_\bullet$.
Notice that the $\tr_n$ are faithful tracial states.
\end{defn}
...
\begin{ex}
By Theorem \ref{thm:ModuleEquivalence}, we can also obtain an equivalent connected right planar module for the subfactor planar algebra $\cT\cL\cJ(d)_\bullet$.
We define $M_n : = \cM_n$ with its $\dag$-algebra structure, and we define $\tr_n : = $ \nn{todo}
\end{ex}
