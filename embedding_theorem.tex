...
\end{align*}
 ...
 To see that $\Tr^\cM$ is compatible with the $\TLJ$ action, as required by Definition \ref{def:PivotalModule}, it helps to structure the category $\TLJ$ similarly to $\cM$. To that end, note that the category of projections of the Markov tower $TLJ_\bullet$ is equivalent to the tensor category $\TLJ$, with the internal tensor product given by an action of $\TLJ$ on itself. That this in fact makes $\TLJ$ a tensor category as well as a pivotal $\TLJ$ module is easy to see, since in $\TLJ$, plenty of isotopy is available, unlike the category $\cM$, which is not a-priori a planar algebra or module. Then the morphism $\coev_{[n]}\in\TLJ([0]\to[2n])$ is just $1\in TLJ_n$. % This may be a dumb way to make this argument, although it is true.
 ...
 we obtain the trivial requirement
