...
\begin{remark}
Notice that shifting a Markov tower simply truncates the Bratelli diagram (as defined in \ref{Rem:BratelliDiagram}), and by a similar argument to the one used to show fact \ref{Fact:BratteliPrincipal}, the principal graph is unchanged.
\end{remark}
...
\begin{remark}
We can determine the Bratelli diagram and principal graph for $(pM_np, \tr_n^p, pe_{n+1})$, as follows. 
If $p$ has central support $1$, then the Bratelli diagram is unchanged. 
...
By proceeding up the tower and, at each level, removing those vertices and edges no longer supported from below, we obtatin the Bratelli diagram for $(pM_np, \tr_n^p, pe_{n+1})$.
\end{remark}
...
$\left( M_{kn}, \tr_{kn}, g_{n+1} \right )_{n\geq 0}$ is also a Markov tower. 
	The Bratteli diagram for $\left( M_{kn}, \tr_{kn}, g_{kn} \right )_{n\geq 0}$ can be read off the Bratelli diagram quite easily: the vertices of the level of the Bratteli diagram corresponding to $M_{kn}$ are the same in both towers, while the number of edges between two vertices in the new diagram is the number of upward paths between those vertices in the old diagram. In the case where $k$ is odd, taking the $k$-step basic construction therefore collapses the vertices of each $k$ levels of the principal graph into one level. 
\nn{	Since a vertex of the multistep principal graph may belong to the old stuff in the original Bratteli diagram, the number of edges between adjacent vertices in the multistep principal graph is not simply the number of paths in the original principal graph. In general, there seems to be no obvious procedure for calculating numbers of edges in the multistep principal graph simpler than computing the Bratteli diagram.}
\textcolor{blue}{The number of paths of length $j$ between two vertices is the entry of the adjacency matrix taken to the $j$-th power. -Dave}
	When $k$ is even, we lose the odd part of the principal graph entirely; aside from this, the situation is the same. 
...
\begin{align*}
	...
	% \TLTStart\TLTThrough{n}\TLTThrough{2i}\TLTCapcup{j}\TLTEnd
	\begin{tikzpicture}[baseline=.3cm]
		\draw (.2,0) -- (.2,.8);
		\node at (.2,1) {\scriptsize{$n$}};
		\draw (.4,0) -- (.4,.8);
	...
	\pi(y\circ x)&=d^{-i-j}
	% \TLTStart\TLTThrough{n}\TLTThrough{2i}\TLTCapcup{j}\TLTThrough{2k}\TLTEnd
	\begin{tikzpicture}[baseline=.3cm]
		\draw (.2,0) -- (.2,.8);
		\node at (.2,1) {\scriptsize{$n$}};
		\draw (.4,0) -- (.4,.8);
	...
	\begin{tikzpicture}[baseline=.3cm]
		...
		\node at (1,1) {\scriptsize{$j$}};
		\draw[thick, rounded corners = 5pt] (0,0) rectangle (1.6,.8);
	\end{tikzpicture} % \\ % but remember to restore the &= below
	= \pi(y) \cdot \pi(x)
\end{align*}
...
&=(1\vartriangleleft g)\circ(f\vartriangleleft 1)
	\end{align*}
	It is more straightforward to check that for $f$ and $g$ in $\cM$ and $h$ and $k$ in $\TLJ$, we have $(g\vartriangleleft 1)\circ(f\vartriangleleft 1)=(g\circ f)\vartriangleleft 1$ and $(1\vartriangleleft k)\circ(1\vartriangleleft h)=1\vartriangleleft(k\circ h)$. 
	The latter is essentially trivial. As for the former, we describe a representative case. Suppose $f:[n]_{\cM}\to[n+2i+2j]_{\cM}$ and $g:[n+4i+2j]_{\cM}\to[n+2i]_{\cM}$, where $j=i+\ell$, and suppose $m=k+i+j$. Then 
	\begin{align*}
		...
		\begin{tikzpicture}[baseline=3cm]
			...
			\draw (1.2,.8) arc (-180:0:.4cm);
			\node at (1.2,1) {\scriptsize{$\ell$}};
			\draw (1.4,.8) arc (-180:0:.2cm);
			\node at (1.4,1) {\scriptsize{$i$}};
			\draw[thick, rounded corners = 5pt] (0,0) rectangle (2.3,.8);
		\end{tikzpicture}
		\circ\rho(g)\circ\rho(f)\circ\rho\left(
		\begin{tikzpicture}[baseline=3cm]
			\draw (.2,0) -- (.2,.8);
			\node at (.2,1) {\scriptsize{$n$}};

		\end{tikzpicture}\right)\circ
		\begin{tikzpicture}[baseline=3cm]

		\end{tikzpicture} % todo add E
	\end{align*}
	From there, routine abstract nonsense verifies that defining $f\vartriangleleft g=(f\vartriangleleft1)\circ(1\vartriangleleft g)$ makes $\cdot\vartriangleleft\cdot$ a bifunctor.
