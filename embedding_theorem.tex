...
\newcommand{\TLTThrough}[1]{
	...
	\end{scope}
	\coordinate (TLTlead) at ($ (TLTlead) + (.2,0) $);
}
...
\newcommand{\TLTCapcup}[1]{
	\begin{scope}[shift=(TLTlead)]
		...
		\node at (0,1) {\scriptsize{$#1$}};
	\end{scope}
	\coordinate (TLTlead) at ($ (TLTlead) + (.6,0) $);
}
...
\newcommand{\TLTSnakeR}[2]{
	\begin{scope}[shift=(TLTlead)]
		...
		\draw (.1,0) .. controls ++(90:.3cm) and ++(270:.3cm) .. (.8,.8);
		\draw (.4,0) arc (180:0:.2cm);
		\node at (.1,1) {\scriptsize{$#1$}};
		\node at (.8,1) {\scriptsize{$#2$}};
		\node at (.4,-.2) {\scriptsize{$#1$}};
	\end{scope}
	\coordinate (TLTlead) at ($ (TLTlead) + (1,0) $);
}
...
\newcommand{\TLTSnakeL}[2]{
	\begin{scope}[shift=(TLTlead)]
		...
		\draw (.8,0) .. controls ++(90:.3cm) and ++(270:.3cm) .. (.1,.8);
...
% join a bottom of #2 caps (left) and #3 caps (right) into #4(inside)+#5(outside) caps together on top
\newcommand{\TLTCapJoin}[4]{
 \begin{scope}[shift=(TLTlead)]
		\draw (0,.8) arc (-180:0:.5);
		\draw (0.2,.8) arc (-180:0:.3);
		\draw (0,0) arc (180:0:.2);
		\draw (.6,0) arc (180:0:.2);
		\node at (0,1) {\scriptsize{$#4$}};
		\node at (.2,1) {\scriptsize{$#3$}};
		\node at (0,-.2) {\scriptsize{$#1$}};
		\node at (.6,-.2) {\scriptsize{$#2$}};
	\end{scope}
	\coordinate (TLTlead) at ($ (TLTlead) + (1.2,0) $);
}
...
\newcommand{\TLTCapSplit}[4]{
 \begin{scope}[shift=(TLTlead)]
		...
		\draw (0,.8) arc (-180:0:.2);
		\draw (.6,.8) arc (-180:0:.2);
		\node at (0,-.2) {\scriptsize{$#4$}};
		\node at (.2,-.2) {\scriptsize{$#3$}};
		\node at (0,1) {\scriptsize{$#1$}};
		\node at (.6,1) {\scriptsize{$#2$}};
	\end{scope}
	\coordinate (TLTlead) at ($ (TLTlead) + (1.2,0) $);
}
...
%$ 
%
%and let ${}^n g^j_k = {{}^n h^j_k}^{\ast}$.
%\item 
%${}^n r^{i+j}=d^{-(i+j)}
%\begin{tikzpicture}[baseline=0, yscale = -1]
...
%I changed the statement of the lemma to help get the ball going.
%-D}
We calculate
\begin{align*}
x\cdot
...
So 
$$
\pi(y\circ x)=E_{n+i+j}(yx \cdot \TLPlusPIso{n}{i}{j}) \cdot \TLCapJoin{n}{i}{j}{j}{i} \cdot \TLTStart\TLTThrough{n}\TLTThrough{2i}\TLTThrough{2j}\TLTCapcup{k}\TLTEnd
$$ 
But then Lemma \ref{lem:LeftKink} immediately gives that:
$$
\pi(y\circ x)= yx \cdot \TLTStart\TLTThrough{n}\TLTCapcup{i}\TLTCapcup{j}\TLTThrough{2k}\TLTEnd \cdot \TLTStart\TLTThrough{n}\TLTThrough{2i}\TLTThrough{2j}\TLTCapcup{k}\TLTEnd 
$$
Which is $\pi(y) \circ \pi(x)$.
 
\eqref{UPdown} 
Let $x:[n]\rightarrow [n+2i+2j]$ and $y:[n+2i+2j]\rightarrow [n+2i]$. 
Then 
$$
\pi(y\circ x)=E_{n+i}(yx \TLMinusPIso{n}{i}{j})\TLTStart\TLTThrough{n}\TLTCapcup{i}\TLTCapcup{j}\TLTCapcup{k}\TLTEnd
$$ 
First observe that using lemma \ref{lem:LeftKink} we have:
$$
E_{n+i}(E_{n+2i}(yx\TLMinusPIso{n}{i}{j}))\TLTStart\TLTThrough{n}\TLTCapcup{i}\TLTEnd= (E_{n+2i}(yx\TLMinusPIso{n}{i}{j})\TLTStart\TLTThrough{n}\TLTCapcup{i}\TLTEnd
$$
And since $E_{n+i}=E_{n+i}\circ E_{n+2i}$ we have that:

$$
\pi(y\circ x)= (E_{n+2i}(yx\TLMinusPIso{n}{i}{j})\TLTStart\TLTThrough{n}\TLTCapcup{i}\TLTCapcup{j}\TLTCapcup{k}\TLTEnd
$$
Now observe that $yx\TLMinusPIso{n}{i}{j}\in M_{n+2i+j}$ and we know from proposition \ref{prop:MultistepJonesProjections} we know that $d^{-j}\TLTStart\TLTThrough{n}\TLTThrough{2i}\TLTCapcup{j}\TLTEnd$ implements $E^{n+2i+j}_{n+2i}$ so:

$$
\pi(y\circ x)=\TLTStart\TLTThrough{n}\TLTThrough{2i}\TLTCapcup{j}\TLTThrough{2k}\TLTEnd \cdot yx \cdot \TLMinusPIso{n}{i}{j} \cdot \TLTStart\TLTThrough{n}\TLTCapcup{i}\TLTCapcup{j}\TLTCapcup{k}\TLTEnd = \pi(y) \circ \pi(x)
$$

\eqref{downUP} Let $x:[n]\rightarrow [n+2i+2j]$ and $y:[n+2i+2j]\rightarrow [n+2i]$
Then:
$$
\pi(y\circ x)=y\cdot \TLPlusPIso{n}{i}{j} \cdot x \cdot \TLTStart\TLTThrough{n}\TLTThrough{2i}\TLTCapcup{j}\TLTCapcup{k}\TLTEnd
$$
and
$$
\pi(y)\circ \pi(x)= y\cdot  \TLCapJoin{n}{i}{j}{j}{i} \cdot x \TLTStart\TLTThrough{n}\TLTThrough{2i}\TLTThrough{2j}\TLTCapcup{k}\TLTEnd
$$
but these are equal as:
$$
y\cdot \TLPlusPIso{n}{i}{j} \cdot x \cdot \TLTStart\TLTThrough{n}\TLTThrough{2i}\TLTCapcup{j}\TLTCapcup{k}\TLTEnd= y\cdot \TLPlusPIso{n}{i}{j} \cdot\TLTStart\TLTThrough{n}\TLTThrough{2i}\TLTCapcup{j}\TLTThrough{k}\TLTEnd \cdot x \cdot  \TLTStart\TLTThrough{n}\TLTThrough{2i}\TLTThrough{2j}\TLTCapcup{k}\TLTEnd = y\cdot  \TLCapJoin{n}{i}{j}{j}{i} \cdot x \cdot \TLTStart\TLTThrough{n}\TLTThrough{2i}\TLTThrough{2j}\TLTCapcup{k}\TLTEnd = \pi(y)\circ \pi(x)  
$$
