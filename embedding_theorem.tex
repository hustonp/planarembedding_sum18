...
The map adding $k$ strings to the left is clearly an injective $*$-algebra map.
By the definition of the action of tangles for canonical planar algebra associated to a strongly Markov inclusion \cite[\S2.3]{MR2812459},
\begin{equation}
...
bxb^*
\end{equation}
for some Pimsner-Popa basis $B$ for $A_{k}$ over $A_0$.
In more detail, 
$$
...
By Proposition \ref{prop:LeftCapping}, the right hand side of \eqref{eq:kStringLeftCap} above is exactly $E^{A_0'}_{A_k'}(x)$;
hence the conditional expectation $E^{A_0'}_{A_k'}: A_0'\cap A_{n+k} \to A_k'\cap A_{n+k}$ is given diagrammatically by
$$
E^{A_0'}_{A_k'}(x)
=
d^{-k}
...
%\end{tikzpicture}\,.
\end{equation*}
which is in the image of the map which adds $k$ strings to the left.
\end{proof}
