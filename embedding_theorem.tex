...
To see that the $\Tr_k$ give a positive module trace, we reduce the general case to the case of endomorphisms as follows: suppose $f\in\cM([n]\to[n+2k])$. Obtain $g\in\cC(1\to(X^?)^{\alttens[2k]})$ from $1\in\cP_{k,\pm}$. Then \[\Tr_n(f^\dag\circ f)=\Tr_{n+2k}((f\vartriangleleft g)^\dag\circ(f\vartriangleleft g))\Tr_{2k}(g^\dag\circ g)^{-1}=d^k\Tr_{n+2k}((f\vartriangleleft g)^\dag\circ(f\vartriangleleft g))\]
