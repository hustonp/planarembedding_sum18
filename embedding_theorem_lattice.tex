...
The proof in \cite{1810.06076} is mostly in the language of tensor categories and their modules.
...
\item
Popa's standard $\lambda$-lattices $A_{\bullet,\bullet}$ of commuting squares \cite{MR1334479}.
\end{enumerate}
...
A \emph{cyclic right lattice module} for a $\lambda$-lattice $A_{\bullet, \bullet}$ with $\lambda=d^{-2} $ is a Markov tower $M_\bullet = (M_n, \tr_n, e_{n+1})_{n\geq 0}$ together with a family of $*$-algebra homomorphisms $\rho^k_n : A_{n,\pm } \to M_k'\cap M_{k+n}$ for $k,n\geq 0$ that map Jones projections to shifted Jones projections, and which are compatible with the inclusions $A_{n,+}\subset A_{n+1,}$ and $A_{n,-} \subset A_{n+1,-}$ and with the conditional expectations.
We refer the reader to Definition \ref{defn:LatticeModule} for precise statements.
...
Let $\cP_\bullet$ be a subfactor planar algebra corresponding to $(\cC, X)$ and $\cA_{\bullet, \bullet}$ as above.
...
In fact, any Markov tower of modulus $d$ is automatically equipped with the structure of a cyclic right lattice module for 
...
In this section, we rapidly recall the definition of a subfactor planar algebra \cite{math.QA/9909027}, its unitary $2\times 2$ multitensor category of projections \cite{MR2811311,1808.00323}, and the associated standard $\lambda$-lattice \cite{MR1334479}.
...
\begin{defn}
A \emph{Markov-Popa lattice} $A_\bullet$ of \emph{modulus $d$}
is a standard $d^{-2}$-sequence/ladder \cite[Def.~1.3]{MR1334479}. 
We give two non-standard equivalent definitions below which emphasize the connection with subfactor planar algebras and Markov towers respectively.

One equivalent definition of a Markov-Popa lattice is that it is the structure obtained from a subfactor planar algebra $\cP_\bullet$ by forgetting the action of most of the shaded planar operad, except:
\begin{itemize}
\item
the tracial von Neumann algebra structure on each $\cP_{n,\pm}$,
\item
the distinguished Jones projections,
\item
the left and right inclusions, and
\item
the left and right conditional expectations.
\end{itemize}

A second equivalent definition of a Markov-Popa lattice $A_\bullet$ is a pair of Markov sequences $(A_{n,\pm}, \tr_{n,\pm}, e_{n+1,\pm})_{n\geq 0}$ of modulus $d$ satisfying the following compatibility axioms:
\begin{enumerate}[label={\rm(L\arabic*)}]
\item
%(cross inclusions)
$A_{n,-} \subset A_{n+1,+}$ for all $n\geq 0$,
with $\tr_{n+1,+}|_{A_{n,-}} = \tr_{n,-}$, 
and these inclusions map $e_{n+1,-}$ to $e_{n+2,+}$,
\item
(commuting square)
$E^{A^{n+1,-}_{A_{n,-}}} \circ E^{A_{n+2,+}}_{A_{n+1,-}} = E^{A_{n+1,+}}_{A_{n,-}} \circ E^{A_{n+2,+}}_{A_{n+1,+}} = E^{A_{n+2,+}}_{A_{n,-}}$,
\item
$E^{A^{n+1,+}}_{A_{n,-}}(e_{n}) = d^{-2}$ for all $n\geq 1$,
\item
$A_{n+1,+} e_{1,+} = A_{n,-} e_{1,+}$
\nn{correct?}
\item (standard)
\end{enumerate}
\end{defn}

Notice that given a subfactor planar algebra $\cP_\bullet$ with loop modulus $d$, we get a Markov-Popa $d^{-2}$ lattice of

\nn{lambda lattices.}

\begin{rem}
We warn the reader that our definition of a $\lambda$-lattice is non-standard; we merely take a subfactor planar algebra and forget some of the structure.
As all standard $\lambda$-lattices in the sense of \cite{MR1334479} arise in this way, we lose no examples.
\end{rem}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Modules for unitary multitensor categories, subfactor planar algebras, and \texorpdfstring{$\lambda$}{lambda}-lattices}\label{ssec:moduledefs}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\nn{Now we consider $\lambda$-lattice modules...}
...
%By a module over $Q_\bullet$, we mean a $\ast$-modMR1976459ule category over the $\ast$-multitensor category $\cC$. Let $\cM$ be a cyclic left $\cC$-module category with generator $m$. 
%Since $\cM$ is indecomposable, we can take $m$ to be simple. 
By \cite{MR1976459}, there is an algebra $A$ internal to $\cC$ such that $\cM\cong\FreeMod_\cC(A)$, the category of free right $A$-modules internal to $\cC$, namely the $\cC$-valued internal $\End$ of $m$. 
Since $m$ is simple, 
$A$ is in fact a Q-system, % Q-system or $Q$-system?
as described in \cite[Rmk.~2.7]{MR3933035}.  
...
By \cite[Thm.~4.1]{MR3933035}, we can again obtain a $3\times 3$ unitary multifusion category $\widetilde{\cC}$ from the $2$-category of bimodules over the algebras $1_{0,+}$, $1_{0,-}$, and $A$, into which
...
Planar algebras were originally introduced in \cite{math.QA/9909027}. We will use the same definitions and conventions as \cite{MR2812459}, \cite{MR2679382}, \cite{math.QA/9909027}, and \cite{MR2972458}. The reader is referred to these references for further information on planar algebras. 
