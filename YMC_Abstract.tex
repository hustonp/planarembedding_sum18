...
In the field of subfactor theory a powerful and well known tool for constructing subfactors are planar algebras, in particular one may be interested in planar subalgebras of a given planar algebra. In 2011 Vaughan Jones and David Penneys showed that a finite depth subfactor planar algebra, $P_{\bullet}$, can embedded in the bipartite graph planar algebra of the principal graph of $P_{\bullet}$. We generalize this result by showing that a finite depth subfactor planar algebra embeds into the bipartite graph planar algebra of the fusion graph for any module over a category constructed from $P_{\bullet}$. Our techniques are largely based on the same tower of algebras approach used by Jones and Penneys, and in particular we construct a tower of algebras to prove the theorem using a generating object in a multifusion category constructed from $P_{\bullet}$, many computations are done using string diagrams for morphisms in the aforementioned multifusion category.
\end{document}